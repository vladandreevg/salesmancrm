...
function getLastCalls(){
	...
	var url = '/content/pbx/asteriskapi/peers.php?action=lastcolls';
	var sms = '';
...
function synchist(){
	...
	var url = '/content/pbx/asteriskapi/cdr.php';
	$.post(url);
...
function inpeers(){
	...
	var isWork = localStorage.getItem("asteriskapiWork");
	var oldtime = parseInt(localStorage.getItem("asteriskapiWorkTime"));
	...
	if(isWork !== 'yes' || adelta > atime){
		...
		localStorage.setItem("asteriskapiWork", 'yes');
		localStorage.setItem("asteriskapiWorkTime", Date.now());

		$.getJSON('/content/pbx/asteriskapi/peers.php?action=getIncoming', function(data){
			...
			.done( function(){
				...
				localStorage.setItem("asteriskapiWork", '');
