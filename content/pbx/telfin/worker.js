...
function getLastCalls(){
	...
	var url = '/content/pbx/telfin/peers.php?action=lastcolls';
	var sms = '';
...
function synchist(){
	...
	url = '/content/pbx/telfin/cdr.php';
	$.post(url);
...
function inpeers(){
	...
	var isWork = localStorage.getItem("gravitelWork");
	var oldtime = parseInt(localStorage.getItem("gravitelWorkTime"));
	...
	if(isWork != 'yes' || adelta > atime){
		...
		localStorage.setItem("gravitelWork", 'yes');
		localStorage.setItem("gravitelWorkTime", Date.now());

		$.getJSON('/content/pbx/telfin/peers.php?action=getIncoming', function(data){
