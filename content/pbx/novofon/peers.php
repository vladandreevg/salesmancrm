...
require_once dirname( __DIR__)."/novofon/sipparams.php";
require_once dirname( __DIR__)."/novofon/mfunc.php";

//переменные из settings.php
$setEntry      = $GLOBALS['setEntry'];
$isEntry       = $GLOBALS['isEntry'];
$modLeadActive = $GLOBALS['modLeadActive'];
$settingsApp   = $GLOBALS['settingsApp'];
...
if ( $action == 'getIncoming' ) {
	...
	//$statuses = $db -> getAll("SELECT * FROM {$sqlname}zadarma_log WHERE (TIMESTAMPDIFF(SECOND, datum, CURRENT_TIMESTAMP) ) < 10 AND extension = '$phone_in' AND identity = '$identity' ORDER BY datum");

	$statuses = $db -> getAll( "SELECT * FROM {$sqlname}novofon_log WHERE (TIMESTAMPDIFF(SECOND, datum, CURRENT_TIMESTAMP) ) < 100 AND status IN ('INCOMING','CALLING','ACCEPTED','COMPLETED','ANSWERED') AND LENGTH(phone) > 5 AND extension = '$phone_in' AND identity = '$identity' ORDER BY datum" );
	...
	foreach ( $statuses as $status ) {
		...
		if ( in_array( strtoupper( $status['status'] ), [
			...
			$inpeers[] = [
				...
				"client"    => current_client( (int)$status['clid'] ),
				"pid"       => (int)$status['pid'],
				"person"    => current_person( (int)$status['pid'] ),
				"phone"     => $status['phone'],
				"state"     => $status['status'],
				"icon"      => $icon,
				"direction" => $direction,
