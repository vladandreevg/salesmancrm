...
function getLastCalls(){
	...
	var url = '/content/pbx/novofon/peers.php?action=lastcolls';
	var sms = '';
...
function synchist(){
	...
	var url = '/content/pbx/novofon/cdr.php';
	$.post(url);
...
function inpeers(){
	...
	var isWork = localStorage.getItem("novofonWork");
	var oldtime = parseInt(localStorage.getItem("novofonWorkTime"));
	...
	if(isWork !== 'yes' || adelta > atime){
		...
		localStorage.setItem("zadarmaWork", 'yes');
		localStorage.setItem("zadarmaWorkTime", Date.now());

		$.getJSON('/content/pbx/novofon/peers.php?action=getIncoming', function(data){
			...
			.done( function(){
				...
				localStorage.setItem("novofonWork", '');
