...
/**
 * скрипт получает уведомления из сервиса Novofon для входящего звонка
 */
...
$ypath = $rootpath."/content/pbx/novofon/";
...
if ($identity == 0) {
	...
	$f = fopen($rootpath."/cash/novofon-worker.log", "a");
...
require_once dirname(__DIR__)."/novofon/novofon-api-v1/lib/Client.php";
...
if ($response['event'] == 'NOTIFY_START') {
	...
	file_put_contents($rootpath."/cash/novofon.log", "$signature - $signatureTest - $api_secret\n");
