...
$bdtimezone = intval($dzz) / 3600;// + $tzone;
...
if ($action == 'lastcolls') {
	...
	$rezult = [
		'INCOMING'   => '<i class="icon-ok-circled green" title="Отвечен"></i>',
		...
		'CANCEL'     => '<i class="icon-minus-circled red" title="Отменен"></i>',
...
if ($action == 'getIncoming') {
	...
	$statuses = $db -> getAll("SELECT * FROM {$sqlname}rostelecom_log WHERE (TIMESTAMPDIFF(SECOND, datum, '".current_datumtime()."') ) < 10 AND extention = '$phone_in' AND type = 'in' AND identity = '$identity' ORDER BY datum DESC");
	...
	foreach ($statuses as $status) {
		...
		//разговоры
		if ($status['status'] == 'ACCEPTED' || $status['status'] == 'COMPLETED' || $status['status'] == 'ANSWERED') {
