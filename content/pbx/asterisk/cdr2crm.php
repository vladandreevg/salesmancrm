...
$params['dateStart'] = urldecode($_REQUEST['dateStart']); if(!$_REQUEST['dateStart']) $params['dateStart'] = $dateStart;
...
if($hash != APIKEY){
	...
	if(@mysql_connect (HOSTBD, USERBD, PASSBD)){
		...
		$result = mysql_query("SELECT * FROM cdr WHERE calldate BETWEEN '".$params['dateStart']."' and '".$params['dateEnd']."' ORDER BY calldate DESC");
		while ($data = mysql_fetch_array($result)){

			$response['data'][] = array(

				"datum" => $data['calldate'],//дата звонка
				"src"   => $data['src'],//источник вызова
				"dst"   => $data['dst'],//пункт назначения вызова
				"res"   => $data['disposition'],//результат ANSWERED, NO ANSWER, BUSY
				"dur"   => $data['duration'],//продолжительность вызова
				"sec"   => $data['billsec'],//Продолжительность вызова с момента ответа на него
				"uid"   => $data['uniqueid'],
				"file"  => $data['recordingfile'],
				"did"   => $data['did']//наш номер, на который поступил звонок

			);
		}

	}
	else $response['data']['error'] = mysql_error();
