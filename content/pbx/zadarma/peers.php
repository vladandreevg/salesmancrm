...
if ( $action == 'getIncoming' ) {
	...
	$statuses = $db -> getAll( "SELECT * FROM {$sqlname}zadarma_log WHERE (TIMESTAMPDIFF(SECOND, datum, CURRENT_TIMESTAMP) ) < 100 AND status IN ('INCOMING','CALLING','ACCEPTED','COMPLETED','ANSWERED') AND LENGTH(phone) > 5 AND extension = '$phone_in' AND identity = '$identity' ORDER BY datum" );
