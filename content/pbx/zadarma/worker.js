...
function getLastCalls(){
	...
	var url = '/content/pbx/zadarma/peers.php?action=lastcolls';
	var sms = '';
...
function synchist(){
	...
	var url = '/content/pbx/zadarma/cdr.php';
	$.post(url);
...
function inpeers(){
	...
	var isWork = localStorage.getItem("zadarmaWork");
	var oldtime = parseInt(localStorage.getItem("zadarmaWorkTime"));
	...
	if(isWork !== 'yes' || adelta > atime){
		...
		$.getJSON('/content/pbx/zadarma/peers.php?action=getIncoming', function(data){
			...
			.done( function(){
				...
				localStorage.setItem("zadarmaWork", '');
