...
if ( $action == 'getIncoming' ) {
	...
	$statuses = $db -> getAll( "SELECT * FROM ".$sqlname."mango_log WHERE (TIMESTAMPDIFF(SECOND, datum, CURRENT_TIMESTAMP) ) < 10 AND extension = '$phone_in' AND identity = '$identity' ORDER BY datum DESC LIMIT 1" );
	//print_r($statuses);
	//print $db -> lastQuery();
	foreach ( $statuses as $status ) {
		...
		//$peersa['debager'] = "\n".$diff." ::: ".current_datumtime()." ::: ".$status['datum']." ::: ".date('Y-m-d H:i:s', mktime(date('H'),date('i'),date('s'), date('m'), date('d'), date('Y')))."\n";
		...
		//разговоры
		if ( $resp['call_state'] == 'Connected' or $resp['call_state'] == 'OnHold' ) {
	...
	if ( $setEntry['enShowButtonCall'] == 'yes' and $isEntry == 'on' )
		$peersa['entry'] = 1;
	if ( $coordinator > 0 )
		$peersa['lead'] = 1;
