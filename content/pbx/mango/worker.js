...
function getLastCalls(){
	...
	var url = '/content/pbx/mango/peers.php?action=lastcolls';
	var sms = '';
...
function synchist(){
	...
	var url = '/content/pbx/mango/cdr.php';
	$.post(url);
...
function inpeers(){
	...
	var isWork = localStorage.getItem("mangoWork");
	var oldtime = parseInt(localStorage.getItem("mangoWorkTime"));
	...
	if(isWork !== 'yes' || adelta > atime){
		...
		localStorage.setItem("mangoWork", 'yes');
		localStorage.setItem("mangoWorkTime", Date.now());

		$.getJSON('/content/pbx/mango/peers.php?action=getIncoming', function(data){
			...
			.done( function(){
				...
				localStorage.setItem("mangoWork", '');
