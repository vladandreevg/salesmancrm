...
foreach ($phones as $phone) {
	...
	$phone_list[] = '<span class="phonec phonenumber '.$ismob.'" data-pid="'.$data['pid'].'" data-clid="'.$clid.'" data-phone="'.prepareMobPhone( $phone ).'">'.formatPhoneUrl( $phone, $clid ).'</span>';
...
<div>
	...
	<div class="flex-container">
		...
		<div class="flex-string wp90" style="max-height: 30vh; overflow-y: auto">
			...
			<div class="row wp100 <?php print ($email_list == '') ? "hidden" : ""; ?>">
			...
			<div class="row wp100 <?php print ($data['category'] == '') ? "hidden" : ""; ?>">
			...
			<div class="row wp100 <?php print ($data['address'] == '') ? "hidden" : ""; ?>">
			...
			<div class="row wp100 <?php print ($data['territoryname'] == '') ? "hidden" : ""; ?>">
			...
			foreach ( $res as $da ) {
				...
				<div class="row ha">
					...
					<div class="column grid-6">
						...
						if ( $da['tel'] != "" ) {
							$phone_list = [];
							$phone      = yexplode( ",", str_replace( ";", ",", str_replace( " ", "", $da['tel'] ) ) );
							for ( $p = 0; $p < count( $phone ); $p++ ) {

								if ( substr( prepareMobPhone( $phone[ $p ] ), 1, 1 ) == '9' )
									$ismob = 'ismob';
								else $ismob = '';
								$phone_list[] = '<span class="phonec phonenumber '.$ismob.'" data-pid="'.$da['pid'].'" data-clid="'.$da['clid'].'" data-phone="'.prepareMobPhone( $phone[ $p ], $clid, $da['pid'] ).'">'.formatPhoneUrl( $phone[ $p ] ).'</span>';
						...
						if ( $da['fax'] != "" ) {
							...
							for ( $p = 0; $p < count( $fax ); $p++ ) {
								...
								if ( substr( prepareMobPhone( $fax[ $p ] ), 1, 1 ) == '9' )
									$ismob = 'ismob';
								else $ismob = '';
								$phone_list[] = '<span class="phonec phonenumber '.$ismob.'" data-pid="'.$da['pid'].'" data-clid="'.$da['clid'].'" data-phone="'.prepareMobPhone( $fax[ $p ] ).'">'.formatPhoneUrl( $fax[ $p ], $clid, $da['pid'] ).'</span>';
						...
						if ( $da['mob'] != "" ) {
							...
							for ( $p = 0; $p < count( $mobi ); $p++ ) {

								if ( substr( prepareMobPhone( $mobi[ $p ] ), 1, 1 ) == '9' )
									$ismob = 'ismob';
								else $ismob = '';
								$phone_list[] = '<span class="phonec phonenumber '.$ismob.'" data-pid="'.$da['pid'].'" data-clid="'.$da['clid'].'" data-phone="'.prepareMobPhone( $mobi[ $p ] ).'">'.formatPhoneUrl( $mobi[ $p ], $clid, $da['pid'] ).'</span>';
						...
						if ( $da['mail'] != "" ) {
							...
							for ( $j = 0; $j < count( $em ); $j++ ) {
								...
								if ( $ymEnable == true && $em[ $j ] != '' )
									$apx = '&nbsp;(<A href="javascript:void(0)" onClick="$mailer.composeCard(\'\',\''.$da['pid'].'\',\''.trim( $em[ $j ] ).'\');" title="Написать сообщение"><i class="icon-mail blue"></i></A>)&nbsp;';
								else $apx = '';
								print '<div>'.link_it( $em[ $j ] ).$apx.'</div>';
			...
			foreach ( $res as $da ) {
				...
				<div class="row ha">
					...
					<div class="column12 grid-8">
						<b class="fs-12"><?= $da['title'] ?></b>&nbsp;
						<a href="javascript:void(0)" onclick="openDogovor('<?= $da['did'] ?>')"><i class="<?= $icon ?>"></i></a>
