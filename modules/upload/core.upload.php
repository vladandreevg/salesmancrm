...
if ( $action == 'edit' ) {
	...
	$did        = $_REQUEST['did'];
	$folder     = untag( $_REQUEST['idcategory'] );
...
if ( $action == "catlist" ) {
	...
	while ($data = $db -> fetch( $result )) {
		...
		if ( $data['shared'] == 'yes' ) {
			$shared = ' <i class="icon-users-1 smalltxt green" title="Общая папка"></i> ';
		}
		else {
			$shared = '';
		}

		print '<a href="javascript:void(0)" data-id="'.$data['idcategory'].'" data-title="'.$data['title'].'" class="fol block mt5 mb5"><div class="ellipsis"><i class="icon-folder blue"></i>&nbsp;'.$shared.$data['title'].'</div></a>';
		...
		while ($da = $db -> fetch( $res )) {
			...
			$shared = $da['shared'] == 'yes' ? ' <i class="icon-users-1 smalltxt green" title="Общая папка"></i> ' : '';

			print '<a href="javascript:void(0)" class="fol block" data-id="'.$da['idcategory'].'" data-title="'.$da['title'].'"><span class="pl10 ellipsis"><div class="strelka w5 mr10"></div><i class="icon-folder blue"></i>&nbsp;'.$shared.$da['title'].'</span></a>';
