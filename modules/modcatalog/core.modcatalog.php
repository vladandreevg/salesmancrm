...
//настройки модуля
$settings            = $db -> getOne(
	"SELECT settings FROM ".$sqlname
	."modcatalog_set WHERE identity = '$identity'"
);
...
$result = $db -> getAll(
	"SELECT * FROM ".$sqlname
	."field WHERE fld_tip='price' AND fld_on='yes' and identity = '$identity' ORDER BY fld_order"
);
...
if ($action == "likeoffer") {
	...
	$users = $db -> getOne(
		"select users from ".$sqlname."modcatalog_offer where id='".$id
	...
	$db -> query(
		"update ".$sqlname."modcatalog_offer set users = '".$nusers
		."' WHERE id = '".$id."' and identity = '$identity'"
	);
	print "Готово";
...
if ($action == "didlist") {
	...
	if ($_REQUEST['t'] == 'zayavka')
		$z = " and did NOT IN (SELECT did FROM ".$sqlname
			."modcatalog_zayavka WHERE identity = '$identity')";
	
	$dids = $db -> getCol(
		"SELECT did FROM ".$sqlname
		."modcatalog_akt where did > 0 $t and isdo != 'yes' and identity = '$identity' ORDER BY datum DESC"
	);
	
	if ($dids[0] > 0)
		$dids = "and did NOT IN (".implode(",", $dids).")"; else $dids = '';
	...
	$result = $db -> getAll(
		"SELECT * FROM ".$sqlname
		."dogovor WHERE clid = '$clid' $dids $z and close != 'yes' and identity = '$identity'"
	);
	foreach ($result as $data) {
		if ($_REQUEST['t'] == 'order') {
			//количество позиций в актах
			$acount = $db -> getOne(
				"SELECT SUM(kol) as count FROM ".$sqlname
				."modcatalog_aktpoz WHERE ida IN (SELECT id FROM ".$sqlname
				."modcatalog_akt WHERE did = '".$data['did']
				."' and tip = 'outcome' and identity = '$identity') and identity = '$identity'"
			);
			
			//количество позиций в спеке
			$scount = $db -> getOne(
				"SELECT SUM(kol) as count FROM ".$sqlname."speca WHERE did = '"
				.$data['did']."' and prid > 0 and identity = '$identity'"
			);
		...
		if ($ap == 1)
			print '<option value="'.$data['did'].'">'.$data['title'].' - '
				.num_format($data['kol']).'</option>';
	}
...
if ($action == "specalist") {
	...
	$result = $db -> getAll(
		"SELECT * FROM ".$sqlname
		."speca WHERE did = '$did' and identity = '$identity'"
	);
	foreach ($result as $data) {
		$res      = $db -> getRow(
			"SELECT * FROM ".$sqlname."price WHERE title = '".$data['title']
			."' and identity = '$identity'"
		);
		$prid     = $res['n_id'];
		...
		$kol_in = $db -> getOne(
			"SELECT SUM(kol) as kol FROM ".$sqlname."modcatalog WHERE prid = '"
			.$prid."' and identity = '$identity'"
		);
		
		$kol_res = $db -> getOne(
			"SELECT SUM(kol) as kol FROM ".$sqlname
			."modcatalog_reserv WHERE prid = '".$data['prid']
			."' and did = '$did' and identity = '$identity'"
		);
		...
		if ($kol_in < (int)$data['kol']) {
			...
			$tt = '<span class="red smalltxt">Нет достаточного количества на складе. На складе - <b>'
				.num_format($kol_in).'</b> позиций.</span>';
		} else {
			$ss = '';
			$tt = '';
		}
		...
		print '
		<tr id="pr_">
			<td><input name="idp[]" id="idp[]" type="hidden" value=""><input name="prid[]" id=prid[]" type="hidden" value="'
			.$prid
			.'"><input name="speca_title[]" type="text" id="speca_title[]" value="'
			.$data['title'].'" style="width:98%" class="requered"/>'.$tt.'</td>
			<td class="text-center"><input name="speca_kol[]" id="speca_kol[]" type="text" value="'
			.num_format($data['kol']).'" style="width:70%; '.$ss.'" class="requered"/></td>
			<td class="text-center"><input name="speca_price[]" id="speca_price[]" type="text" min="1" value="'
			.num_format($data['price']).'" style="width:90%" class="requered"/></td>
...
if ($action == "specalist2") {
	...
	*/
	...
	$filter = ($tip != 'outcome') ? " and ".$sqlname
		."speca.prid NOT IN (SELECT prid FROM ".$sqlname
		."modcatalog_zayavkapoz WHERE did = '$did' and identity = '$identity')" : "";
	...
	foreach ($result as $data) {
		$countKol = $db -> getOne(
			...
			SELECT SUM(kol) 
			FROM ".$sqlname."modcatalog_aktpoz 
				LEFT JOIN ".$sqlname."modcatalog_akt ON ".$sqlname
			."modcatalog_akt.id = ".$sqlname."modcatalog_aktpoz.ida
			WHERE 
		...
		if (($data['kol'] - $countKol) > 0) {
			...
			$price_in = $db -> getOne(
				"SELECT price_in FROM ".$sqlname."price WHERE n_id = '"
				.$data['prid']."' and identity = '$identity'"
			);
			...
			if ($sklad > 0)
				$kol_sklad = $db -> getOne(
						"SELECT SUM(kol) as kol FROM ".$sqlname
						."modcatalog_skladpoz WHERE sklad = '$sklad' and prid = '"
						.$data['prid']."' $pozzi and identity = '$identity'"
					) + 0;
			...
			if ($kol_free < $data['kol']) {
				$ss = 'border:1px solid red';
				$tt = '<span class="red smalltxt">На складе - <b>'.$kol_sklad
					.'</b> позиций (В резерве под сделку - '.$kol_res
					.', В резерве: - '.$kol_res.').</span>';
			...
			$countDeal = $db -> getAll(
				"SELECT SUM(kol) as kol FROM ".$sqlname
				."modcatalog_skladpoz WHERE did = '$did' and identity = '$identity'"
			);
			...
			if ($tip == 'outcome' && $sklad != '') {
				if ($settings['mcSkladPoz'] == 'yes') {
					$dx = ($countDeal > 0) ? " and (did = '".$did
						."' or did is null or did = 0)" : " and did is null";
					
					$re = $db -> getAll(
						"SELECT * FROM ".$sqlname
						."modcatalog_skladpoz WHERE sklad = '$sklad' and prid = '"
						.$data['prid']."' $dx and identity = '$identity'"
					);//and serial != ''
					foreach ($re as $da) {
						$selected = '';
...
if ($action == "specalist3") {
	...
	if ($did > 0 && $idz < 1)
		$idz = $db -> getOne(
			"SELECT id FROM ".$sqlname
			."modcatalog_zayavka WHERE did = '$did' and identity = '$identity'"
		);
	
	$c     = $db -> getRow(
		"SELECT did, conid FROM ".$sqlname
		."modcatalog_zayavka WHERE id = '$idz' and identity = '$identity'"
	);
	$conid = $c['conid'];
	if ($idz > 0)
		$did = $c['did'];
	...
	//формируем запрос
	if ($did > 0 && $idz < 1) {
		$sort = (count($settings['mcPriceCat']) > 0) ? " and ".$sqlname
			."price.pr_cat IN (".yimplode(",", $settings['mcPriceCat'])
			.")" : '';
		...
		*/
		...
		$q      = "SELECT * FROM ".$sqlname
			."modcatalog_zayavkapoz WHERE id > 0 and idz = '$idz' and identity = '$identity'";
		$result = $db -> getAll($q);
		//print_r($result);
	...
	foreach ($result as $data) {
		$res      = $db -> getRow(
			"SELECT * FROM ".$sqlname."price WHERE n_id = '".$data['prid']
			."' and identity = '$identity'"
		);
		$price_in = $res['price_in'];
		$price    = $res['price_1'];
		...
		$kol_in = $db -> getOne(
			"SELECT SUM(kol) as kol FROM ".$sqlname
			."modcatalog_skladpoz WHERE sklad = '$sklad' $pozzi and prid = '"
			.$data['prid']."' and identity = '$identity'"
		);
		...
		$kol_do = ($idz > 0) ? $db -> getOne(
			"SELECT SUM(kol) as kol FROM ".$sqlname
			."modcatalog_aktpoz WHERE prid = '".$data['prid']
			."' and ida IN (SELECT id FROM ".$sqlname
			."modcatalog_akt WHERE idz = '".$idz
			."' and identity = '$identity') and identity = '$identity'"
		) : 0;
		
		//количество в заявке
		$kol_zay = ($idz > 0) ? $db -> getOne(
			"SELECT SUM(kol) as kol FROM ".$sqlname
			."modcatalog_zayavkapoz WHERE prid = '".$data['prid']
			."' and idz = '$idz' and identity = '$identity'"
		) : 0;
	...
	if ($did > 0) {
		...
		*/
		...
		foreach ($result as $data) {
			...
			$kol_in = ( ! empty($sklads)) ? $db -> getOne(
					"SELECT SUM(kol) as kol FROM ".$sqlname
					."modcatalog_skladpoz WHERE sklad IN (".yimplode(
						",",
						$sklads
					)
					.") $pozzi and prid = '$data[prid]' and identity = '$identity'"
				) + 0 : 0;
			
			//количество на складе под сделку
			$kol_din = $db -> getOne(
					"SELECT SUM(kol) as kol FROM ".$sqlname
					."modcatalog_skladpoz WHERE did = '$did' $pozzi and prid = '$data[prid]' and identity = '$identity'"
				) + 0;
			...
			if ($did > 0) {
				$q = "
					...
					FROM ".$sqlname."modcatalog_zayavkapoz
						LEFT JOIN ".$sqlname."modcatalog_zayavka ON ".$sqlname
					."modcatalog_zayavka.id = ".$sqlname."modcatalog_zayavkapoz.idz
					WHERE
						".$sqlname."modcatalog_zayavkapoz.prid = '"
					.$data['prid']."' and
			...
			if ($kol_in < pre_format($data['kol'])) {
				$ss = 'border:1px solid red';
				$tt = '<span class="red smalltxt">На складе - <b>'.$kol_in
					.'</b> позиций (с учетом резерва под сделку).</span>';
			}
...
if ($action == "pozfromsklad") {
	...
	$res = $db -> query(
		"SELECT * FROM ".$sqlname
		."modcatalog_skladpoz WHERE sklad = '$sklad' and status != 'out' and identity = '$identity'"
	);
...
if ($action == "filedelete") {
	...
	$files = $db -> getOne(
		"select files from ".$sqlname."modcatalog WHERE id='".$id
		."' and identity = '$identity'"
	);
	...
	$db -> query(
		"update ".$sqlname."modcatalog set files = '".$nxfiles."' where id = '"
		.$id."' and identity = '$identity'"
	);
...
if ($action == "export") {
	...
	$result = $db -> getAll(
		"SELECT * FROM ".$sqlname
		."field WHERE fld_tip='price' and identity = '$identity' ORDER BY fld_order"
	);
	foreach ($result as $data) {
		$dname[$data['fld_name']] = $data['fld_title'];
	}
	...
	$appendix = $db -> getCol(
		"SELECT name FROM ".$sqlname
		."modcatalog_fieldcat WHERE identity = '$identity' ORDER by ord"
	);
	if (count($appendix) > 0)
		$appendix = ";".yimplode(";", $appendix);
	
	$otchet[0] = explode(
		";",
		convertcsv(
			"Артикул;Категория;Наименование;Статус;Количество;Ед.изм.;"
			.$dname['price_in'].";".$dname['price_1'].";".$dname['price_2'].";"
			.$dname['price_3'].";".$dname['price_4'].";".$dname['price_5']
			.";НДС;Описание краткое;Описание полное;Изображение 1;Изображение 2;Изображение 3;Изображение 4;Изображение 5"
			.$appendix
		)
	);
	...
	if ($idcategory) {
		...
		$isSub = $db -> getOne(
			"SELECT sub FROM ".$sqlname."price_cat WHERE idcategory='"
			.$idcategory."' and identity = '$identity'"
		);
		
		if ($isSub
			== 0) {//если это головная категория, то выбираем все записи и из подкатегорий
			
			$sub  = $db -> getCol(
				"SELECT idcategory FROM ".$sqlname."price_cat WHERE sub='"
				.$idcategory."' and identity = '$identity'"
			);
			$subb = yimplode(",", $sub);
	...
	if ($word != '')
		$sort .= $sort." and ((artikul LIKE '%".$word."%') or (title LIKE '%"
			.$word."%') or (descr LIKE '%".$word."%'))";
	
	if ($statuss != '') {
		//print "SELECT n_id FROM ".$sqlname."modcatalog where status IN (".$statuss.") and identity = '$identity'";
		$sta = $db -> getCol(
			"SELECT prid FROM ".$sqlname."modcatalog where status= '".$statuss
			."' and identity = '$identity'"
		);
		$sta = yimplode(",", $sta);
	...
	$p      = 1;
	$result = $db -> getAll(
		"SELECT * FROM ".$sqlname."price WHERE n_id > 0 ".$sort
		." and identity = '$identity' ORDER BY pr_cat"
	);
	foreach ($result as $data) {
		$res   = $db -> getRow(
			"select title, sub from ".$sqlname."price_cat where idcategory='"
			.$data['pr_cat']."' and identity = '$identity'"
		);
		$cat   = $res["title"];
		$idsub = $res["sub"];
		
		$res      = $db -> getRow(
			"select * from ".$sqlname."modcatalog where prid='".$data['n_id']
			."' and identity = '$identity'"
		);
		...
		$otchet[$p] = [
			...
			num_format($data['price_1']),
			num_format($data['price_2']),
			num_format($data['price_3']),
			num_format($data['price_4']),
			num_format($data['price_5']),
			num_format($data['nds']),
			clnall(untag(convertcsv($data['descr']))),
			convertcsv($contentt),
			$files[0]['file'],
			$files[1]['file'],
			$files[2]['file'],
		...
		//$otchet[$p] = array($data['artikul'],$cat,$data['title'],$status,$kol,$data['edizm'],num_format($data['price_in']),num_format($data['price_1']),num_format($data['price_2']),num_format($data['price_3']),num_format($data['price_4']),num_format($data['price_5']),num_format($data['nds']),clnall($data['descr']),$contentt,$files[0]['file'],$files[1]['file'],$files[2]['file'],$files[3]['file'],$files[3]['file']);
		...
		foreach ($re as $da) {
			...
			$value          = $db -> getOne(
				"SELECT value FROM ".$sqlname."modcatalog_field WHERE n_id = '"
				.$data['n_id']."' and pfid = '".$da['id']
				."' and identity = '$identity'"
			);
			$otchet[$p][$k] = convertcsv($value);
...
if ($action == "editdop_on") {
	...
	if ($params['id'] > 0) {
		...
		$result               = $db -> getRow(
			"SELECT * FROM ".$sqlname."modcatalog_dop WHERE id = '"
			.$params['id']."' and identity = '$identity'"
		);
		...
		$db -> query(
			"update ".$sqlname."modcatalog_dop set summa = '".$params['summa']
			."', datum = '".$params['datum']."', content = '".$params['content']
			."', clid = '".$params['clid']."' WHERE id = '".$params['id']
			."' and identity = '$identity'"
		);
		...
		$db -> query(
			"insert into ".$sqlname
			."modcatalog_dop (id,prid,datum,content,summa,clid,iduser,identity) value (null,'"
			.$params['n_id']."','".$params['datum']."','".$params['content']
			."','".$params['summa']."','".$params['clid']
			."','$iduser1','$identity')"
		);
...
if ($action == "editaktundo_on") {
	...
	$order = $db -> getRow(
		"SELECT * FROM ".$sqlname."modcatalog_akt WHERE id = '".$id
		."' and identity = '$identity'"
	);
...
if ($action == "editzayavka_on") {
	...
	unset($_REQUEST['idp']);
	unset($_REQUEST['speca_title']);
	unset($_REQUEST['speca_kol']);
	unset($_REQUEST['prid']);
	...
	$mes .= (count($result['error']) > 0) ? "<br>Ошибки:<br>".implode(
			"<br>",
			$result['error']
		) : '';
...
if ($action == "import_on") {
	...
	$url   = $rootpath.'/files/'.$fpath
		.$_COOKIE['url_catalog'];//файл для расшифровки
	...
	for ($i = 0; $i < count($field); $i++) {
		if (strstr($field[$i], 'catalog') != false and strstr(
				$field[$i],
				'status'
			) == false) {
			$c = str_replace("catalog:", "", $field[$i]);
			if ($c == 'title')
				$clx = $i; //индекс названия позиции
			
			$indexs['catalog'][]  = $i;//массив ключ поля -> номер столбца
			$names['catalog'][$i] = $c;//массив номер столбца -> индекс поля
		}
		if (strstr($field[$i], 'price') != false and strstr(
				$field[$i],
				'category'
			) == false) {
			$c = str_replace("price:", "", $field[$i]);
			if ($c == 'title')
				$clx = $i; //индекс названия позиции
		...
		if (strstr($field[$i], 'profile') != false) {
			$c = str_replace("profile:", "", $field[$i]);
			
			$indexs['profile'][]  = $i;//массив ключ поля -> номер столбца
			$names['profile'][$i] = $c;//массив номер столбца -> индекс поля
		}
		if (strstr($field[$i], 'status') != false) {
			$c = str_replace("status:", "", $field[$i]);
			
			$indexs['status'][$c] = $i;//массив ключ поля -> номер столбца
		}
		if (strstr($field[$i], 'category') != false) {
			$c = str_replace("category:", "", $field[$i]);
	...
	for ($i = 1; $i < count($data); $i++) {
		...
		//обработаем отрасль
		if (enc_detect($data[$i][$indexs['status']['title']]) != '') {
			$status[$i] = strtr(
				enc_detect($data[$i][$indexs['status']['title']]),
				$statuses
			);
			...
			$status[$i] = "'".$status[$i]
				."'";//массив для формирования запроса для вставки новой позиции
		...
		if (enc_detect($data[$i][$indexs['category']['title']]) != '') {
			$prcat = $db -> getCol(
				"SELECT title FROM ".$sqlname
				."price_cat WHERE identity = '$identity' ORDER BY title"
			);
			
			//сопоставляем id раздела текущей позиции, если нет создаем.
			if (in_array(
				enc_detect(untag($data[$i][$indexs['category']['title']])),
				$prcat
			)) {
				//если такое название уже сужествует, то сопоставляем id
				$idcat[$i] = $db -> getOne(
					"SELECT idcategory FROM ".$sqlname
					."price_cat WHERE title = '".enc_detect(
						untag($data[$i][$indexs['category']['title']])
					)."' and identity = '$identity'"
				);
			} else {
				$db -> query(
					"insert into ".$sqlname
					."price_cat (`idcategory`, `title`, `identity`) values(null, '"
					.enc_detect(untag($data[$i][$indexs['category']['title']]))
					."','$identity')"
				);
				$idcat[$i] = $db -> insertId();
		...
		for ($j = 0; $j < count($indexs['price']); $j++) {
			if ($data[$i][$indexs['price'][$j]] != '') {
				...
				if ($names['price'][$indexs['price'][$j]] == 'descr') {
					$qup[$i][] = $names['price'][$indexs['price'][$j]]." = '"
						.htmlspecialchars($data[$i][$indexs['price'][$j]])."'";
					$queryp    = "'".htmlspecialchars(
							$data[$i][$indexs['price'][$j]]
						)."'";
				} elseif ($names['price'][$indexs['price'][$j]] == 'nds') {
					$qup[$i][] = $names['price'][$indexs['price'][$j]]." = '"
						.intval($data[$i][$indexs['price'][$j]])."'";
					$queryp    = "'".intval($data[$i][$indexs['price'][$j]])
						."'";
				} elseif (in_array(
					$names['price'][$indexs['price'][$j]],
					$isPrice
				)) {
					$qup[$i][] = $names['price'][$indexs['price'][$j]]." = '"
						.pre_format($data[$i][$indexs['price'][$j]])."'";
					$queryp    = "'".pre_format($data[$i][$indexs['price'][$j]])
						."'";
				} else {
					$qup[$i][] = $names['price'][$indexs['price'][$j]]." = '"
						.untag($data[$i][$indexs['price'][$j]])."'";
		...
		for ($j = 0; $j < count($indexs['catalog']); $j++) {
			if ($data[$i][$indexs['catalog'][$j]] != '') {
				...
				if ($names['catalog'][$indexs['catalog'][$j]] == 'content') {
					$quc[$i][] = $names['catalog'][$indexs['catalog'][$j]]
						." = '".htmlspecialchars(
							$data[$i][$indexs['catalog'][$j]]
						)."'";
					$queryc    = "'".htmlspecialchars(
							$data[$i][$indexs['catalog'][$j]]
						)."'";
				} elseif (in_array(
					$names['catalog'][$indexs['catalog'][$j]],
					$isFile
				)) {
					//$quc[$i][] = '';
					//$queryc = '';
					...
					$quc[$i][] = $names['catalog'][$indexs['catalog'][$j]]
						." = '".intval($data[$i][$indexs['catalog'][$j]])."'";
					$queryc    = "'".intval($data[$i][$indexs['catalog'][$j]])
						."'";
				} else {
					$quc[$i][] = $names['catalog'][$indexs['catalog'][$j]]
						." = '".untag($data[$i][$indexs['catalog'][$j]])."'";
		...
		$n_id = $db -> getOne(
			"select n_id from ".$sqlname."price where title='".untag(
				enc_detect($data[$i][$clx])
			)."' and identity = '$identity'"
		);
		...
		if ($n_id > 0) {
			if ($db -> query(
				"update ".$sqlname."price set ".$stringPrice." WHERE n_id='"
				.$n_id."' and identity = '".$identity."'"
			))
				$upd1++;
		} else {
			if ($db -> query(
				"INSERT INTO ".$sqlname."price (".$namePrice2
				.",identity) VALUES (NULL, ".$stringPrice2.",'".$identity."')"
			)) {
				$new1++;
				$n_id = $db -> insertId();
			}
		...
		$prid = $db -> getOne(
			"select prid from ".$sqlname."modcatalog where prid='".$n_id
			."' and identity = '$identity'"
		);
		...
		if ($prid > 0 and $stringCatalog != ' ') {
			if ($db -> query(
				"update ".$sqlname."modcatalog set ".$stringCatalog
				." WHERE prid='".$n_id."' and identity = '$identity'"
			))
				$upd2++;
		} else {
			if ($nameCatalog2 != ' ') {
				if ($db -> query(
					"insert into ".$sqlname."modcatalog (".$nameCatalog2
					.",identity) values (null, $n_id, ".$stringCatalog2
					.",'$identity')"
				))
					$new2++;
			}
		...
		for ($j = 0; $j < count($indexs['profile']); $j++) {
			if ($data[$i][$indexs['profile'][$j]] != '') {
				$profile[$names['profile'][$indexs['profile'][$j]]] = untag(
					$data[$i][$indexs['profile'][$j]]
				);
			}
		}
		...
		if (count($profile) > 0) {
			...
			foreach ($profile as $key => $value) {
				...
				$idp = $db -> getOne(
					"SELECT id FROM ".$sqlname
					."modcatalog_fieldcat where pole = '".$key
					."' and identity = '$identity'"
				);
				
				$idc = $db -> getOne(
					"SELECT id FROM ".$sqlname."modcatalog_field where pfid = '"
					.$idp."' and n_id = '$n_id' and identity = '$identity'"
				);
				...
				if ($idc < 1 and $idp > 0) {
					...
					if ($db -> query(
						"insert into ".$sqlname
						."modcatalog_field (id,pfid,n_id,value,identity) values(null, '"
						.$idp."', '".$n_id."', '".$value."','$identity')"
					))
						$new3++;
					...
					if ($db -> query(
						"update ".$sqlname."modcatalog_field set value = '"
						.$value."' where n_id = '".$n_id."' and pfid = '".$idp
						."' and identity = '$identity'"
					))
						$upd3++;
				}
...
if ($action == "mass_do") {
	...
	if ($newcat > 0 and $doAction == 'pMove') {//если пеоемещаем
		...
		if ($isSelect == 'doAll') {//все
			...
			if ($idcategory) {
				...
				if ($isSub
					...
					$sub = $db -> getCol(
						"SELECT idcategory FROM ".$sqlname
						."price_cat WHERE sub='".$idcategory
						."' and identity = '$identity'"
					);
			...
			if ($word != '')
				$ss = $ss." and ((artikul LIKE '%".$word."%') or (title LIKE '%"
					.$word."%') or (descr LIKE '%".$word."%'))";
			...
			$res = $db -> getAll(
				"SELECT n_id FROM ".$sqlname
				."price WHERE n_id > 0 $ss and identity = '$identity' ORDER BY title"
			);
			foreach ($res as $datas) {
				if ($db -> query(
					"update ".$sqlname."price set pr_cat='$newcat' WHERE n_id='"
					.$datas['n_id']."' and identity = '$identity'"
				))
					$good++;
			}
		}
		if ($isSelect == 'doSel' and $ids[0] > 0) {//выбранные
			
			for ($i = 0; $i < count($ids); $i++) {
				if ($ids[$i] > 0) {
					if ($db -> query(
						"update ".$sqlname
						."price set pr_cat='$newcat' WHERE n_id='".$ids[$i]
						."' and identity = '$identity'"
					))
						$good++;
				}
	...
	if ($doAction == 'pDele') {//если удаляем
		...
		if ($isSelect == 'doAll') {//все
			...
			$res = $db -> getAll(
				"SELECT n_id FROM ".$sqlname."price WHERE n_id > 0 ".$ss
				." and identity = '$identity'"
			);
			foreach ($res as $datas) {
				if ($db -> query(
					"delete from ".$sqlname."price where n_id = '"
					.$datas['n_id']."' and identity = '$identity'"
				))
					$good++;
				
				if ($db -> query(
					"delete from ".$sqlname."modcatalog where prid = '"
					.$datas['n_id']."' and identity = '$identity'"
				))
					$good++;
			}
		...
		if ($isSelect == 'doSel' and $ids[0] > 0) {//выбранные
			...
			for ($i = 0; $i < count($ids); $i++) {
				if ($ids[$i] > 0) {
					if ($db -> query(
						"delete from ".$sqlname."price where n_id = '".$ids[$i]
						."' and identity = '$identity'"
					))
						$good++;
...
if ($action == "upload") {
	...
	if ($_FILES['file']['name'] != '') {
		...
		//загружаем из файла
		$cur_ext = texttosmall(
			end(explode(".", basename($_FILES['file']['name'])))
		);
		if ($cur_ext != 'xls') {
			$err = "Ошибка при загрузке файла!\nОшибка: Допускаются только файлы в формате XLS";
		} else {
			$url = '../../files/'.$fpath.translit(
					str_replace(" ", "", basename($_FILES['file']['name']))
				);
		...
		if ($file != '') {
			...
			$data = $datas -> dumptoarray(
			);//получили двумерный массив с данными
...
if ($action == 'exportPoz') {
	...
	if (count($sklad) > 0)
		$sort .= $sqlname."modcatalog_skladpoz.sklad IN (".yimplode(",", $sklad)
			.") and";
	if (count($sstatus) > 0)
		$sort .= $sqlname."modcatalog_skladpoz.status IN (".yimplode(
				",",
				$sstatus,
				"'"
			).") and";
	...
	$query = "
		...
		FROM ".$sqlname."modcatalog_skladpoz 
			LEFT JOIN ".$sqlname."price ON ".$sqlname
		."modcatalog_skladpoz.prid = ".$sqlname."price.n_id
			LEFT JOIN ".$sqlname."modcatalog_sklad ON ".$sqlname
		."modcatalog_skladpoz.sklad = ".$sqlname."modcatalog_sklad.id
		WHERE 
	...
	while ($data = $db -> fetch($result)) {
		$category = $db -> getOne(
			"select title from ".$sqlname
			."price_cat where idcategory = '$data[category]'"
		);
		...
		if ($settings['mcSkladPoz'] == "yes") {
			$order_in  = $db -> getOne(
				"select number from ".$sqlname
				."modcatalog_akt where id = '$data[idorder_in]'"
			);
			$order_out = $db -> getOne(
				"select number from ".$sqlname
				."modcatalog_akt where id = '$data[idorder_out]'"
			);
			
			array_push(
				$otchet[$i],
				$data['serial'],
				strtr($data['status'], $statuses),
				$data['date_in'],
				$order_in,
				$data['date_out'],
				$order_out
