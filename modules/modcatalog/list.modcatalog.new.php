...
$additionalword = str_replace(" ", "%", (string)$_POST['additionalword']);
...
if ($tar == 'catalog') {
	...
	// колонки, доступные для перестановки (без учета фиксированных checkbox/действий)
	$mcArtikulOn = ( $settings['mcArtikul'] == 'yes' );
	$price1On    = in_array('price_1', $don);
	$mcKolEditOn = ( $settings['mcKolEdit'] == 'yes' and in_array($iduser1, (array)$settings['mcCoordinator']) );
	$priceTitle  = (string)$dname['price_1'];
	$additionalword = str_replace(" ", "%", (string)$_REQUEST['additionalword']);

	$columnfile = $rootpath.'/cash/sklad_columns_'.$iduser1.'.json';

	if (file_exists( $columnfile )) {
		$columns = $columnfile;
	}
	else {
		$columns = $rootpath.'/cash/columns_default_sklad.json';
	}

	$def = json_decode(file_get_contents($columns), true);
	...
	$columnDefs = [
		"artikul" => [
			"enabled" => $mcArtikulOn,
			"class"   => "yw{$defaultWidth['artikul']} text-left drag--accept",
		...
		"image"   => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['image']} text-center drag--accept",
			"th"      => '<div class="inline">֍</div>',
			"td"      => function ($item) {
				return '<TD '.$item['image'].' data-id="image"></TD>';
			},
		],
		"card"    => [
			"enabled" => true,
			"class"   => "w{$defaultWidth['card']} text-center drag--accept",
			"th"      => '<div class="inline">〠</div>',
			"td"      => function ($item) {
				return '<TD class="text-center" data-id="card"><A href="javascript:void(0)" onclick="window.open(\'/card.modcatalog?n_id='.$item['id'].'\')"><i class="icon-archive broun"></i></A></TD>';
			},
		],
		"title"   => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['title']} text-left drag--accept",
			...
			"td"      => function ($item) use ( $additionalword ) {
				$cat = ( $item['cat'] !== '' ) ? '<span class="smalltxt blue">'.h($item['cat']).'</span>' : '';
				$match = '';
		...
		"price_1" => [
			"enabled" => $price1On,
			"class"   => "yw{$defaultWidth['price_1']} text-left drag--accept",
			"th"      => '<div class="ellipsis hand" id="x-price_1" onclick="changesort(\'price_1\')" title="Изменить порядок вывода">'.h($priceTitle).', '.h($valuta).'</div>',
			"td"      => function ($item) use ($mcKolEditOn, $valuta) {
				$edit = $mcKolEditOn ? '&nbsp;<a href="javascript:void(0)" onclick="doLoad(\'/modules/modcatalog/form.modcatalog.php?n_id='.$item['id'].'&action=editone&tip=price\');" title="Изменить" class="dleft1 gray"><i class="icon-pencil smalltxt"></i></a>' : '';
				return '
			<TD class="text-right" title="'.h($item['price1'].' '.$valuta.'/'.$item['edizm']).'" data-id="pricee_1">
				<span>
					<b class="blue">'.h($item['price1']).'</b>
					'.$edit.'
				</span>
			</TD>
			';
		...
		"nds" => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['nds']} text-left drag--accept",
		...
		"kol"     => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['kol']} text-center drag--accept",
			"th"      => '<b>Кол-во</b>',
			"td"      => function ($item) {
				return '<TD class="text-right Bold" data-id="kol"><span>'.h($item['kol']).'</span></TD>';
			},
		],
		"edizm"   => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['edizm']} text-right drag--accept",
			"th"      => '<b>Ед.изм.</b>',
			"td"      => function ($item) {
				return '<TD data-id="edizm"><div class="ellipsis1 text-right">'.$item['edizm'].'</div></TD>';
			},
		],
		"res"     => [
			"enabled" => true,
			"class"   => "yw{$defaultWidth['res']} text-center drag--accept",
			"th"      => '<b>Резерв</b>',
			"td"      => function ($item) {
				return '<TD class="text-right Bold" data-id="res"><span>'.$item['res'].'</span></TD>';
			},
		],
		"content" => [
			"enabled" => true,
	...
	if ( $additionalword != '' ) {
		$sort .= " and prc.n_id IN (SELECT n_id FROM {$sqlname}modcatalog_field WHERE value LIKE '%$additionalword%' and identity = '$identity' and pfid IN (SELECT id FROM {$sqlname}modcatalog_fieldcat WHERE identity = '$identity'))";
	}
	...
	foreach ($result as $da) {
		...
		// доп.характеристики позиции
		$fdata = $db -> getAll( "SELECT f.id, f.pfid, f.value, fc.name as fieldname FROM {$sqlname}modcatalog_field f LEFT JOIN {$sqlname}modcatalog_fieldcat fc ON f.pfid = fc.id WHERE f.n_id = '".$da['id']."' and f.identity = '$identity'" );

		// поля, по которым найдено совпадение при поиске по доп.характеристикам
		$additionalMatch = [];
		if ( $additionalword != '' ) {
			$additionalMatch = $db -> getAll( "SELECT f.id, f.pfid, f.value, fc.name as fieldname FROM {$sqlname}modcatalog_field f LEFT JOIN {$sqlname}modcatalog_fieldcat fc ON f.pfid = fc.id WHERE f.n_id = '".$da['id']."' and f.value LIKE '%$additionalword%' and f.pfid IN (SELECT id FROM {$sqlname}modcatalog_fieldcat WHERE identity = '$identity') and f.identity = '$identity'" );
		}
