...
CREATE TABLE `app_activities` (
    ...
    `color` VARCHAR(7) NULL  COMMENT 'цвет в RGB',
    `icon` VARCHAR(100) NULL  COMMENT 'иконка',
    `resultat` TEXT NULL  COMMENT 'список готовых реультатов, разделенный ;',
    `isDefault` VARCHAR(6) NULL  COMMENT 'признак дефолтности',
...
CREATE TABLE `app_budjet` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'название расхода-дохода',
    ...
    `do` VARCHAR(3) NULL  COMMENT 'признак того, что расход проведен',
    `rs` VARCHAR(20) NULL  COMMENT 'id расчетного счета из таблицы _mycomps_recv.id',
    `rs2` VARCHAR(20) NULL  COMMENT 'id расчетного счета (используется при перемещении средств)',
    ...
    `invoice` VARCHAR(255) NULL  COMMENT 'номер счета',
...
CREATE TABLE `app_budjet_bank` (
    ...
    `number` VARCHAR(50) NULL  COMMENT 'номер документа',
    `datum` DATE NULL  COMMENT 'дата проводки',
    `mon` VARCHAR(2) NULL  COMMENT 'месяц',
    `year` VARCHAR(4) NULL  COMMENT 'год',
    `tip` VARCHAR(10) NULL  COMMENT 'направление расхода - dohod, rashod',
    `title` VARCHAR(255) NULL  COMMENT 'название расхода',
    ...
    `fromRS` VARCHAR(20) NULL  COMMENT 'р.с. плательщика',
    `fromINN` VARCHAR(12) NULL  COMMENT 'инн плательщика',
    `to` TEXT NULL  COMMENT 'название получателя',
    `toRS` VARCHAR(20) NULL  COMMENT 'р.с. получателя',
    `toINN` VARCHAR(12) NULL  COMMENT 'инн получателя',
...
CREATE TABLE `app_budjet_cat` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'название',
    `tip` VARCHAR(10) NULL  COMMENT 'тип (расход-доход)',
    `clientpath` INT NULL ,
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_budjetlog` (
    ...
    `status` VARCHAR(10) NULL  COMMENT 'статус расхода',
...
CREATE TABLE `app_callhistory` (
    ...
    `did` VARCHAR(50) NULL  COMMENT 'номер телефона наш если в src добавочный',
...
CREATE TABLE `app_category` (
    ...
    `title` VARCHAR(250) NULL  COMMENT 'название отрасли',
...
CREATE TABLE `app_clientcat` (
    ...
    `uid` VARCHAR(30) NULL  COMMENT 'уникальный идентификатор для внешних ИС, используется при интеграции через API',
    `title` VARCHAR(250) NULL  COMMENT 'название',
    `idcategory` INT NULL  DEFAULT '0' COMMENT 'отрасль (_category.idcategory)',
    `iduser` VARCHAR(10) NULL  DEFAULT '0' COMMENT 'ответственный сотрудник (_user.iduser)',
    `clientpath` INT NULL  DEFAULT '0' COMMENT 'источник клиента (_clientpath.id)',
    `des` TEXT NULL  COMMENT 'описание',
    `address` TEXT NULL  COMMENT 'адрес',
    `phone` VARCHAR(250) NULL  COMMENT 'телефон',
    `fax` VARCHAR(250) NULL  COMMENT 'факс',
    `site_url` VARCHAR(250) NULL  COMMENT 'сайт',
    `mail_url` VARCHAR(250) NULL  COMMENT 'email',
    `trash` VARCHAR(10) NULL  DEFAULT 'no' COMMENT 'yes, если находится в виртуальной корзине (типа холодный клиент)',
    `fav` VARCHAR(20) NULL  DEFAULT 'no' COMMENT 'избранное',
    `pid` INT NULL  DEFAULT '0' COMMENT 'основной контакт (pid в таблице _personcat.pid)',
    `head_clid` INT NULL  DEFAULT '0' COMMENT 'головная организация',
    `scheme` TEXT NULL  COMMENT 'схема принятия решений',
    `tip_cmr` VARCHAR(255) NULL  COMMENT 'тип отношений из табл. _relations.title ',
    `territory` INT NULL  DEFAULT '0' COMMENT 'территория из табл. _territory_cat.idcategory',
    `input1` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input2` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input3` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input4` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input5` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input6` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input7` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input8` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input9` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `input10` VARCHAR(255) NULL  COMMENT 'пользовательские поля',
    `date_create` TIMESTAMP NULL  DEFAULT CURRENT_TIMESTAMP COMMENT 'дата добавления записи',
    `date_edit` TIMESTAMP NULL  COMMENT 'дата последнего изменения',
    `creator` INT NULL  DEFAULT '0' COMMENT 'iduser кто создал (id пользователя user.iduser)',
    `editor` INT NULL  DEFAULT '0' COMMENT 'iduser кто отредактировал последним (id пользователя user.iduser)',
    `recv` TEXT NULL  COMMENT 'набор реквизитов',
    `dostup` VARCHAR(255) NULL  COMMENT 'DEPRECATED',
    `last_dog` DATE NULL  COMMENT 'дата последней закрытой сделки',
    `last_hist` DATETIME NULL  COMMENT 'дата последней записи в историю активностей (history)',
    `type` VARCHAR(100) NULL  DEFAULT 'client' COMMENT 'тип записи (client,person,contractor,partner,concurent)',
...
CREATE TABLE `app_clientpath` (
    ...
    `isDefault` VARCHAR(6) NULL  COMMENT 'Дефолтный признак',
    `utm_source` VARCHAR(255) NULL  COMMENT 'Связка с источником',
    `destination` VARCHAR(12) NULL  COMMENT 'Связка с номером телефона',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_comments` (
    ...
    `datum` TIMESTAMP NULL  COMMENT 'время записи',
...
CREATE TABLE `app_complect_cat` (
    ...
    `title` VARCHAR(200) NULL  COMMENT 'название контрольной точки',
    `corder` INT NULL  DEFAULT '0' COMMENT 'порядок вывода',
    `dstep` INT NULL  DEFAULT '0' COMMENT 'привязка к этапу сделки _dogcategory.idcategory',
...
CREATE TABLE `app_contract` (
    ...
    `datum` DATETIME NULL  COMMENT 'дата добавления',
    `number` VARCHAR(255) NULL  COMMENT 'номер документа',
    `datum_start` DATE NULL  COMMENT 'период действия старт',
    `datum_end` DATE NULL  COMMENT 'период действия стоп',
    `des` TEXT NULL  COMMENT 'описание',
    `clid` INT NULL  DEFAULT '0' COMMENT 'клиент _clientcat.clid',
    `payer` INT NULL  DEFAULT '0' COMMENT 'плательщик _clientcat.clid плательщик по сделке м.б. другим',
    `pid` INT NULL  DEFAULT '0' COMMENT 'контакт _personcat.pid',
    `did` INT NULL  DEFAULT '0' COMMENT 'сделка _dogovor.did',
    `ftitle` VARCHAR(255) NULL  COMMENT 'имена файлов оригинальные (,)',
    `fname` VARCHAR(250) NULL  COMMENT 'имена файлов, которые храним в папке files (,)',
    `ftype` VARCHAR(250) NULL  COMMENT 'типы файлов (,)',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'автор _user.iduser',
    `title` TEXT NULL  COMMENT 'название документа',
    `idtype` INT NULL  DEFAULT '0' COMMENT 'тип документа _contract_type.id',
    `crid` INT NULL  DEFAULT '0' COMMENT 'связанный счет credit.crid (для актов сервисных сделок)',
    `mcid` INT NULL  DEFAULT '0',
    `signer` INT NULL  DEFAULT '0' COMMENT 'id подписанта',
...
CREATE TABLE `app_contract_temp` (
    `id` INT NOT NULL auto_increment,
    `typeid` INT NULL  DEFAULT '0' COMMENT 'тип документа _contract_type.id',
    `title` VARCHAR(255) NULL  COMMENT 'название шаблона',
    `file` VARCHAR(255) NULL  COMMENT 'прикрепленный файл шаблона',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_contract_type` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  COMMENT 'название документа',
    `type` VARCHAR(255) NULL  COMMENT 'внутренний тип get_akt, get_aktper, get_dogovor, invoice',
    ...
    `format` VARCHAR(255) NULL  COMMENT 'шаблон формата номера',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_credit` (
    ...
    `did` INT NULL  DEFAULT '0' COMMENT 'dogovor.did',
    `clid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid',
    `pid` INT NULL  DEFAULT '0' COMMENT 'personcat.pid',
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата счета',
    `datum_credit` DATE NULL  COMMENT 'дата оплаты план',
    `summa_credit` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'сумма оплаты',
    `nds_credit` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'ндс по счету',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'автор счета user.iduser',
    `idowner` INT NULL  DEFAULT '0' COMMENT 'Ответственный за сделку',
    `do` VARCHAR(5) NULL  DEFAULT 'no' COMMENT 'отметка об оплате',
    `invoice` VARCHAR(20) NULL  COMMENT 'Номер счета',
    `invoice_chek` VARCHAR(40) NULL  COMMENT 'Номер договора',
    `invoice_date` DATE NULL  COMMENT 'Дата факт',
    `rs` INT NULL  DEFAULT '0' COMMENT 'Расчетный счет _mycomps_recv.id',
    `tip` VARCHAR(255) NULL  COMMENT 'тип счета',
    `template` INT NULL  DEFAULT '0' COMMENT 'Шаблон счета',
    `suffix` TEXT NULL  COMMENT 'вставка из шаблона (суффикс счета)',
    `signer` INT NULL  DEFAULT '0' COMMENT 'id подписанта',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`crid`),
   INDEX `do` (`do`),
...
CREATE TABLE `app_currency` (
    ...
    `name` VARCHAR(50) NULL  COMMENT 'название валюты',
    `view` VARCHAR(10) NULL  COMMENT 'отображаемое название валюты',
...
CREATE TABLE `app_currency_log` (
    ...
    `iduser` VARCHAR(10) NULL  COMMENT 'сотрудник, который выполнил действие',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`),
   INDEX `id` (`id`)
...
CREATE TABLE `app_customsettings` (
    ...
    `tip` VARCHAR(50) NULL  COMMENT 'тип (для идентификации) параметра',
    `params` TEXT NULL  COMMENT 'параметры (json)',
    `iduser` INT NULL ,
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_deal_anketa_base` (
    ...
    `pole` VARCHAR(10) NULL  COMMENT 'id поля',
...
CREATE TABLE `app_deal_anketa_list` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'Название анкеты',
...
CREATE TABLE `app_direction` (
    ...
    `isDefault` VARCHAR(5) NULL  COMMENT 'признак дефолтности',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_dogovor` (
    ...
    `uid` VARCHAR(30) NULL  COMMENT 'идентификтор для внешних ИС',
    `idcategory` INT NULL  DEFAULT '0' COMMENT 'тип сделки _dogcategory.idcategory',
    `clid` INT NULL  DEFAULT '0' COMMENT 'клиент _clientcat.clid',
    `payer` INT NULL  DEFAULT '0' COMMENT 'плательщик _clientcat.clid',
    `pid` INT NULL  DEFAULT '0' COMMENT 'контакт (DEPRECATED) personcat.pid',
    `datum` DATE NULL  COMMENT 'дата создания',
    `autor` INT NULL  DEFAULT '0' COMMENT 'автор сделки _user.iduser',
    `datum_plan` DATE NULL  COMMENT 'плановая дата реализации',
    `title` TEXT NULL  COMMENT 'название сделки',
    `content` TEXT NULL  COMMENT 'описание',
    `tip` VARCHAR(100) NULL  DEFAULT '0' COMMENT 'тип сделки _dogtips.tid',
    `kol` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'бюджет сделки, сумма',
    `close` VARCHAR(5) NULL  DEFAULT 'no' COMMENT 'признак закрытой сделки',
    `lat` FLOAT(10,6) NULL  COMMENT 'DEPRECATED',
    `lan` FLOAT(10,6) NULL  COMMENT 'DEPRECATED',
    `adres` TEXT NULL  COMMENT 'адрес',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'ответственный _user.iduser',
    `datum_izm` DATE NULL  COMMENT 'дата изменения',
    `datum_close` DATE NULL  COMMENT 'дата закрытия',
    `sid` INT NULL  DEFAULT '0' COMMENT 'статус закрытия сделки _dogstatus.sid',
    `kol_fact` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'факт. сумма при закрытии',
    `des_fact` TEXT NULL  COMMENT 'комментрарий при закрытии сделки',
    `coid` VARCHAR(12) NULL  COMMENT 'конкруент, выигравший сделку _clientcat.clid',
    `co_kol` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'сумма конкурента',
    `coid1` TEXT NULL  COMMENT 'список конкурентов, участвующих в сделке, _clientcat.clid с типом concurent и разделением ;',
    `coid2` VARCHAR(12) NULL  COMMENT 'DEPRECATED',
    `dog_num` TEXT NULL  COMMENT 'связанный договор _contract.deid :) неожиданно!',
    `marga` DOUBLE(20,2) NULL  DEFAULT '0.00',
    `calculate` VARCHAR(4) NULL  COMMENT 'признак того, что в сделке включено составление спецификации (yes-no)',
    `isFrozen` INT NULL  DEFAULT '0',
    `datum_start` DATE NULL  COMMENT 'период сделки. старт',
    `datum_end` DATE NULL  COMMENT 'период сдеки. финиш',
    `pid_list` VARCHAR(255) NULL  COMMENT 'список контактов, _personcat.pid с разделением (;)',
    `partner` VARCHAR(100) NULL  COMMENT 'DEPRECATED',
    `zayavka` VARCHAR(200) NULL  COMMENT 'поле заявки',
    `ztitle` VARCHAR(255) NULL  COMMENT 'текст заявки',
    `mcid` INT NULL  DEFAULT '0' COMMENT 'Привязка к своей компании _mycomps.id',
    `direction` INT NULL  DEFAULT '0' COMMENT 'Направление _direction.id',
    `idcurrency` INT NULL  DEFAULT '0' COMMENT 'id валюты',
    `idcourse` INT NULL  DEFAULT '0' COMMENT 'id курса по сделке',
    `akt_date` DATE NULL  COMMENT 'DEPRECATED',
    `akt_temp` VARCHAR(200) NULL  COMMENT 'DEPRECATED',
    `lid` INT NULL  DEFAULT '0' COMMENT 'связь с заявкой _leads.id',
    `input1` VARCHAR(512) NULL ,
    `input2` VARCHAR(512) NULL ,
    `input3` VARCHAR(512) NULL ,
    `input4` VARCHAR(512) NULL ,
    `input5` VARCHAR(512) NULL ,
    `input6` VARCHAR(512) NULL ,
    `input7` VARCHAR(512) NULL ,
    `input8` VARCHAR(512) NULL ,
    `input9` VARCHAR(512) NULL ,
    `input10` VARCHAR(512) NULL ,
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`did`),
   INDEX `identity` (`identity`),
   INDEX `iduser` (`iduser`),
...
CREATE TABLE `app_dogprovider` (
    `id` INT NOT NULL auto_increment,
    `did` INT NULL  DEFAULT '0' COMMENT 'Сделка _dogovor.did',
    `conid` INT NULL  DEFAULT '0' COMMENT 'Поставщик _clientcat.clid (tip = contragent)',
    `partid` INT NULL  DEFAULT '0' COMMENT 'Партнер _clientcat.clid (tip = partner)',
    `summa` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'сумма',
    `status` VARCHAR(20) NULL  COMMENT 'проведен расход в бюджете или нет',
    `bid` INT NULL  DEFAULT '0' COMMENT 'Связь с записью в журнале расходов _budjet.id',
    `recal` INT NULL  DEFAULT '0' COMMENT '0 - учитывать в расчете маржи, 1 - не учитывать',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_dogstatus` (
    ...
    `result_close` VARCHAR(5) NULL  COMMENT 'Результат закрытия: lose - Проигрыш; win - Победа',
    `content` TEXT NULL  COMMENT 'описание',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`sid`)
...
CREATE TABLE `app_doubles` (
    ...
    `ids` VARCHAR(100) NULL  COMMENT 'список всех id, упомятутых в list',
...
CREATE TABLE `app_entry_poz` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'название позиции',
...
CREATE TABLE `app_field` (
    ...
    `fld_tip` VARCHAR(10) NULL  COMMENT 'тип поля - client, person, price, dogovor',
    `fld_name` VARCHAR(10) NULL  COMMENT 'имя поля в БД',
    `fld_title` VARCHAR(100) NULL  COMMENT 'название поля для интерфейса',
    ...
    `fld_stat` VARCHAR(10) NULL  COMMENT 'можно ли поле выключить',
    `fld_temp` VARCHAR(255) NULL  COMMENT 'тип поля - input, select...',
    `fld_var` TEXT NULL  COMMENT 'вариант готовых ответов',
    `fld_sub` VARCHAR(10) NULL  COMMENT 'доп.разделение для карточек клиентов - клиент, поставщик, партнер..',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`fld_id`)
...
CREATE TABLE `app_file` (
    ...
    `ftitle` VARCHAR(255) NULL  COMMENT 'оригинальное название файла',
    `fname` VARCHAR(255) NULL  COMMENT 'имя, хранимое в системе',
    ...
    `folder` TEXT NULL  COMMENT 'Папка _file_cat.idcategory',
    `datum` DATETIME NULL ,
    `size` INT NULL ,
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`fid`)
...
CREATE TABLE `app_file_cat` (
    `idcategory` INT NOT NULL auto_increment,
    `subid` INT NULL  DEFAULT '0' COMMENT 'родительская папка idcategory',
    `title` VARCHAR(250) NULL  COMMENT 'название категории',
...
INSERT INTO `app_file_cat` VALUES ('11','0','Файлы почты','no','1');#%%
...
CREATE TABLE `app_group` (
    ...
    `service` VARCHAR(60) NULL  COMMENT 'Связка с сервисом _services.name',
    `idservice` VARCHAR(100) NULL  COMMENT 'id группы во внешнем сервисе',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_grouplist` (
    ...
    `service` VARCHAR(255) NULL  COMMENT 'Имя сервиса _services.name',
    `user_name` VARCHAR(255) NULL  COMMENT 'имя подписчика',
    `user_email` VARCHAR(255) NULL  COMMENT 'email подписчика',
    `user_phone` VARCHAR(15) NULL  COMMENT 'телефон подписчика',
    `tags` TEXT NULL  COMMENT 'тэги',
    `status` VARCHAR(100) NULL  COMMENT 'статус подписчика',
    `availability` VARCHAR(100) NULL  COMMENT 'доступность подписчика',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`),
...
CREATE TABLE `app_history` (
    ...
    `clid` INT NULL  DEFAULT '0' COMMENT 'Клиент _clientcat.clid',
    `pid` VARCHAR(100) NULL  COMMENT 'Контакты _personcat.pid в виде списка с разделителем (;)',
    `did` INT NULL  DEFAULT '0' COMMENT 'Сделка _dogovor.did',
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата создания',
    `datum_izm` DATETIME NULL  COMMENT 'дата изменения',
    `des` TEXT NULL  COMMENT 'описание',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'Автор _user.iduser',
    `iduser_izm` INT NULL  DEFAULT '0' COMMENT 'Редактор _user.iduser',
    `tip` VARCHAR(50) NULL  COMMENT 'Тип активности _activities.title',
    `fid` VARCHAR(255) NULL  COMMENT 'Список файлов _files.fid в виде списка с разделением (;)',
    `uid` VARCHAR(100) NULL  COMMENT 'UniqueID из астериска, связь с _callhistory.uid',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`cid`),
   INDEX `clid` (`clid`),
   INDEX `pid` (`pid`),
...
CREATE TABLE `app_kb` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'название папки',
    `share` VARCHAR(5) NULL  COMMENT 'DEPRECATED',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`idcat`)
...
CREATE TABLE `app_kbtags` (
    `id` INT NOT NULL auto_increment,
    `name` VARCHAR(255) NULL  COMMENT 'тэг',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_knowledgebase` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'название статьи',
    `content` MEDIUMTEXT NULL  COMMENT 'содержание статьи',
    `count` INT NULL  COMMENT 'число просмотров',
    `active` VARCHAR(5) NULL  COMMENT 'признак черновика',
...
CREATE TABLE `app_kpi` (
    ...
    `year` INT NULL  COMMENT 'Год',
    `period` VARCHAR(10) NULL  COMMENT 'Период расчета',
...
CREATE TABLE `app_kpibase` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  COMMENT 'Название показателя',
    `tip` VARCHAR(20) NULL  COMMENT 'Тип показателя',
...
CREATE TABLE `app_leads` (
    `id` INT NOT NULL auto_increment,
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата создания записи',
    `datum_do` DATETIME NULL  COMMENT 'дата обработки',
    `status` INT NULL  DEFAULT '0' COMMENT 'статус 0 => Открыт, 1 => В работе, 2 => Обработан, 3 => Закрыт',
    `rezult` INT NULL  DEFAULT '0' COMMENT 'результат обработки 1 => Спам, 2 => Дубль, 3 => Другое, 4 => Не целевой',
    `title` VARCHAR(255) NULL  COMMENT 'Имя',
    ...
    `ip` VARCHAR(16) NULL ,
    `city` VARCHAR(100) NULL  COMMENT 'адрес почтовый',
    `country` VARCHAR(255) NULL  COMMENT 'страна',
    `timezone` VARCHAR(5) NULL ,
    `iduser` INT NULL  DEFAULT '0' COMMENT '_user.iduser',
    `clientpath` INT NULL  DEFAULT '0' COMMENT '_clientpath.id',
    `pid` INT NULL  DEFAULT '0' COMMENT '_personcat.pid',
    `clid` INT NULL  DEFAULT '0' COMMENT '_clientcat.clid',
    `did` INT NULL  DEFAULT '0' COMMENT '_dogovor.did',
    `partner` INT NULL  DEFAULT '0' COMMENT '_clientcat.clid',
    `muid` VARCHAR(255) NULL  COMMENT 'uid письма в почтовом ящике',
...
CREATE TABLE `app_leads_utm` (
    ...
    `utm_source` VARCHAR(255) NULL  COMMENT 'Название источника',
    `utm_url` VARCHAR(500) NULL  COMMENT 'Адрес целевой страницы',
    `utm_medium` VARCHAR(255) NULL  COMMENT 'Канал кампании',
    `utm_campaign` VARCHAR(255) NULL  COMMENT 'Название кампании',
    `utm_term` VARCHAR(255) NULL  COMMENT 'Ключевые слова, фраза',
    `utm_content` VARCHAR(255) NULL  COMMENT 'Доп.описание кампании',
    `site` VARCHAR(255) NULL  COMMENT 'Адрес сайта',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_mail` (
    ...
    `theme` VARCHAR(255) NULL  COMMENT 'тема сообщения',
    `tip` VARCHAR(20) NULL  COMMENT 'тип рассылки (от пользователя или от компании)',
    ...
    `do` VARCHAR(5) NULL  COMMENT 'признак проведения рассылки',
...
CREATE TABLE `app_mail_tpl` (
    ...
    `name_tpl` VARCHAR(250) NULL  COMMENT 'имя шаблона',
...
CREATE TABLE `app_modcatalog` (
    `id` INT NOT NULL auto_increment,
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `idz` INT NULL  DEFAULT '0' COMMENT 'modcatalog_zayavka.id',
    `content` MEDIUMTEXT NULL  COMMENT 'описание позиции',
    `datum` DATETIME NULL  COMMENT 'дата',
    ...
    `sklad` INT NULL  DEFAULT '0' COMMENT 'modcatalog_sklad.id',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'user.iduser',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_akt` (
    `id` INT NOT NULL auto_increment,
    `did` INT NULL  DEFAULT '0' COMMENT 'dogovor.did',
    `tip` VARCHAR(100) NULL  COMMENT 'приходный или расходный',
    `number` INT NULL  DEFAULT '0' COMMENT 'номер ордера',
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата создания',
    `clid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid',
    `posid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid (id поставщика)',
    `man1` VARCHAR(255) NULL  COMMENT ' для расходного сдал, для приходного принял',
    `man2` VARCHAR(255) NULL  COMMENT ' для расходного принял, для приходного сдал',
    `isdo` VARCHAR(5) NULL  COMMENT 'проведен или нет',
    `cFactura` VARCHAR(20) NULL  COMMENT '№ счета-фактуры поставщика',
    `cDate` DATE NULL  COMMENT 'Дата счета-фактуры',
    `sklad` INT NULL  DEFAULT '0' COMMENT 'modcatalog_sklad.id',
    `idz` INT NULL  DEFAULT '0' COMMENT 'id заявки (modcatalog_zayavka.id)',
    `identity` INT NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_aktpoz` (
    `id` INT NOT NULL auto_increment,
    `ida` INT NULL  DEFAULT '0' COMMENT 'id акта в таблице modcatalog_akt (modcatalog_akt.id)',
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `price_in` DOUBLE NULL  DEFAULT '0' COMMENT 'цена по ордеру',
...
CREATE TABLE `app_modcatalog_dop` (
    `id` INT NOT NULL auto_increment,
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `bid` INT NULL  DEFAULT '0' COMMENT 'по-моему не используется',
    `datum` DATE NULL ,
    `content` TEXT NULL  COMMENT 'наименование доп. затрат',
    `summa` DOUBLE NULL  COMMENT 'стоимость доп. затрат',
    `clid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'user.iduser',
    `identity` INT NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_field` (
    `id` INT NOT NULL auto_increment,
    `pfid` INT NULL  DEFAULT '0' COMMENT 'modcatalog_fieldcat.id',
    `n_id` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `value` VARCHAR(255) NULL  COMMENT 'значение доп поля для данной продукции',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`),
   INDEX `value` (`value`)
...
CREATE TABLE `app_modcatalog_fieldcat` (
    `id` INT NOT NULL auto_increment,
    `name` VARCHAR(255) NULL  COMMENT 'название доп поля',
    `tip` VARCHAR(10) NULL  COMMENT 'тип вывода: поле ввода, поле текста, список выбора, чекбоксы, радиокнопки и разделитель',
    `value` TEXT NULL  COMMENT 'выбираемы значения',
    `ord` INT NULL  DEFAULT '0' COMMENT 'порядковый номер поля в списке доп. полей',
    `pole` VARCHAR(10) NULL ,
    `pwidth` INT NULL  DEFAULT '0' COMMENT 'ширина поля',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_log` (
    `id` INT NOT NULL auto_increment,
    `dopzid` INT NULL  DEFAULT '0' COMMENT 'modcatalog_dop.id',
    `datum` DATETIME NULL  COMMENT 'дата изменения',
    `tip` VARCHAR(255) NULL  COMMENT 'где происходит измениение: catalog, dop, kol, price, status',
    `new` TEXT NULL  COMMENT 'было ',
    `old` TEXT NULL  COMMENT 'стало',
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'id пользователя user.iduser',
    `identity` INT NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_offer` (
    `id` INT NOT NULL auto_increment,
    `datum` DATETIME NULL  COMMENT 'дата предложения',
    `datum_end` DATETIME NULL ,
    `status` INT NULL  DEFAULT '0' COMMENT 'статус 0-актуальная, 1-закрытая',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'user.iduser',
    `content` TEXT NULL  COMMENT 'коментарий предложения',
    `des` TEXT NULL  COMMENT 'данные по НДС, названию предложения, сумме',
    `users` TEXT NULL  COMMENT 'user.iduser с разделением ; принявшие предложение снабжения (голосование)',
    `prid` INT NULL  DEFAULT '0' COMMENT 'id созданной позиции (price.n_id)',
    `identity` INT NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_reserv` (
    ...
    `status` VARCHAR(30) NULL  COMMENT 'статус резерва (действует/снят)',
    `idz` INT NULL  DEFAULT '0' COMMENT 'id заявки, по которой ставили резерв (modcatalog_zayavka.id)',
    `ida` INT NULL  DEFAULT '0' COMMENT 'id акта в таблице modcatalog_akt (modcatalog_akt.id)',
...
CREATE TABLE `app_modcatalog_set` (
    ...
    `ftp` TEXT NULL  COMMENT 'настройки ftp',
    `identity` INT NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_sklad` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  COMMENT 'название склада',
    `mcid` INT NULL  DEFAULT '0' COMMENT 'привязка к компании (mycomps.id)',
    `isDefault` VARCHAR(5) NULL  DEFAULT 'no' COMMENT 'склад по умолчанию для каждой компании',
    `identity` INT NULL ,
   PRIMARY KEY (`id`),
   UNIQUE KEY `id` (`id`)
...
CREATE TABLE `app_modcatalog_skladmovepoz` (
    `id` INT NOT NULL auto_increment,
    `idm` INT NULL  DEFAULT '0' COMMENT 'id группы перемещения (modcatalog_skladmove.id)',
    `idp` INT NULL  DEFAULT '0' COMMENT 'id позиции из таблицы modcatalog_skladpoz',
    `prid` INT NULL  DEFAULT '0' COMMENT 'id позиции прайса (price.n_id)',
    `kol` DOUBLE(20,4) NULL  DEFAULT '1.0000' COMMENT 'количество для общего учета',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_modcatalog_skladpoz` (
    ...
    `serial` VARCHAR(255) NULL  COMMENT 'серийный номер',
...
CREATE TABLE `app_modcatalog_zayavka` (
    `id` INT NOT NULL auto_increment,
    `number` VARCHAR(50) NULL  DEFAULT '0' COMMENT 'номер заявки',
    `did` INT NULL  DEFAULT '0' COMMENT 'dogovor.did',
    `datum` DATETIME NULL  COMMENT 'дата заявки',
    `datum_priority` DATE NULL  COMMENT 'желаемая дата (срочность)',
    `datum_start` DATETIME NULL  COMMENT 'дата начало выполнения заявки',
    `datum_end` DATETIME NULL  COMMENT 'дата окончания выполнения заявки',
    `status` INT NULL  DEFAULT '0' COMMENT '0 - создана, 1-в работе, 2- выполнено, 3-отмена',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'автор user.iduser',
    `sotrudnik` INT NULL  DEFAULT '0' COMMENT 'ответственный user.iduser',
    `content` TEXT NULL  COMMENT 'коментарий заявки',
    `rezult` TEXT NULL ,
    `des` TEXT NULL  COMMENT 'заполнение доп полей',
    `isHight` VARCHAR(3) NULL  DEFAULT 'no',
    `cInvoice` VARCHAR(20) NULL ,
...
CREATE TABLE `app_modcatalog_zayavkapoz` (
    `id` INT NOT NULL auto_increment,
    `idz` INT NULL  DEFAULT '0' COMMENT 'id заявки (odcatalog_zayavka.id)',
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
...
CREATE TABLE `app_modules` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(100) NULL  COMMENT 'название модуля',
    `content` TEXT NULL  COMMENT 'какие сделаны настройки модуля',
    `mpath` VARCHAR(255) NULL ,
...
CREATE TABLE `app_multisteps` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  COMMENT 'Мультиворонка. Название цепочки этапов',
    `direction` INT NULL  COMMENT 'id from _direction Направление',
    `tip` INT NULL  COMMENT 'tid from _dogtips Тип сделки',
    `steps` VARCHAR(255) NULL  COMMENT 'Набор этапов',
    `isdefault` VARCHAR(5) NULL  COMMENT 'id этапа по умолчанию',
    `identity` INT NULL  DEFAULT '1',
   PRIMARY KEY (`id`)
)  COMMENT='База мультиворонки'  ENGINE=InnoDB DEFAULT CHARSET='utf8';#%%
...
CREATE TABLE `app_mycomps` (
    ...
    `dir_name` VARCHAR(255) NULL  COMMENT 'в лице руководителя',
    `dir_signature` VARCHAR(255) NULL  COMMENT 'подпись руководителя',
    `dir_status` TEXT NULL  COMMENT 'должность руководителя',
    `dir_osnovanie` TEXT NULL  COMMENT 'действующего на основаии',
    `innkpp` VARCHAR(255) NULL  COMMENT 'инн/кпп',
    `okog` VARCHAR(255) NULL  COMMENT 'окпо/огрн',
    `stamp` VARCHAR(255) NULL  COMMENT 'файл с факсимилией',
    `logo` VARCHAR(255) NULL  COMMENT 'файл с логотипом',
    `identity` INT NULL  DEFAULT '1' COMMENT 'идентификатор аккаунта (id записи в таблице settings)',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_mycomps_recv` (
    `id` INT NOT NULL auto_increment,
    `cid` INT NULL  DEFAULT '0' COMMENT 'id компании mycomps.id',
    `title` TEXT NULL  COMMENT 'назваине р.с',
    `rs` VARCHAR(50) NULL  COMMENT 'р.с',
...
CREATE TABLE `app_mycomps_signer` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'Имя подписанта',
    `status` VARCHAR(255) NULL  COMMENT 'Должность',
    `signature` VARCHAR(255) NULL  COMMENT 'Подпись',
    `osnovanie` VARCHAR(255) NULL  COMMENT 'Действующий на основании',
    `stamp` VARCHAR(255) NULL  COMMENT 'Файл факсимилье',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`),
   INDEX `mcid` (`mcid`)
...
CREATE TABLE `app_notes` (
    ...
    `author` INT NULL  DEFAULT '0' COMMENT 'id пользователя, создавшего заметку',
    `pin` INT NULL  DEFAULT '0' COMMENT 'признак важности заметки',
    `text` VARCHAR(180) NULL  COMMENT 'Текст заметки',
    `identity` INT NULL  DEFAULT '1' COMMENT 'идентификатор аккаунта (id записи в таблице settings)',
   PRIMARY KEY (`id`),
   UNIQUE KEY `id` (`id`)
...
CREATE TABLE `app_notify` (
    ...
    `title` VARCHAR(255) NULL  COMMENT 'заголовок уведомления',
    `content` TEXT NULL  COMMENT 'содержимое уведомления',
    `url` TEXT NULL  COMMENT 'ссылка на сущность',
    `tip` VARCHAR(50) NULL  COMMENT 'тип связанной записи',
...
CREATE TABLE `app_office_cat` (
    `idcategory` INT NOT NULL auto_increment,
    `title` VARCHAR(250) NULL  COMMENT 'адрес офиса',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`idcategory`)
...
CREATE TABLE `app_otdel_cat` (
    ...
    `title` VARCHAR(250) NULL  COMMENT 'название отдела',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`idcategory`)
...
CREATE TABLE `app_personcat` (
    ...
    `clid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid',
    `ptitle` VARCHAR(250) NULL  COMMENT 'должность контакта',
    `person` VARCHAR(250) NULL  COMMENT 'фио контакта',
    `tel` VARCHAR(250) NULL  COMMENT 'телефон контакта',
    `fax` VARCHAR(250) NULL  COMMENT 'факс контакта',
    `mob` VARCHAR(250) NULL  COMMENT 'мобильный контакта',
    `mail` VARCHAR(250) NULL  COMMENT 'email контакта',
    `rol` TEXT NULL  COMMENT 'роль контакта',
    `social` TEXT NULL  COMMENT 'ссылки в интернете на контакт',
    `iduser` VARCHAR(12) NULL  DEFAULT '0' COMMENT 'куратор контакта',
    `clientpath` INT NULL  DEFAULT '0' COMMENT 'clientpath.id',
    `loyalty` INT NULL  DEFAULT '0' COMMENT 'loyal_cat.idcategory',
    `input1` VARCHAR(255) NULL ,
    `input2` VARCHAR(255) NULL ,
    `input3` VARCHAR(255) NULL ,
    `input4` VARCHAR(255) NULL ,
    `input5` VARCHAR(255) NULL ,
    `input6` VARCHAR(255) NULL ,
    `input7` VARCHAR(255) NULL ,
    `input8` VARCHAR(255) NULL ,
    `input9` VARCHAR(255) NULL ,
    `input10` VARCHAR(512) NULL ,
    `date_create` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата добавления клиента',
    `date_edit` TIMESTAMP NULL  COMMENT 'дата изменения контакта',
    `creator` INT NULL  DEFAULT '0' COMMENT 'сотрудник добавивший клиента user.iduser',
    `editor` INT NULL  DEFAULT '0' COMMENT 'сотрудник который внес изменение user.iduser',
...
CREATE TABLE `app_plugins` (
    ...
    `version` VARCHAR(10) NULL  COMMENT 'Установленная версия плагина',
...
CREATE TABLE `app_price` (
    ...
    `artikul` VARCHAR(255) NULL  COMMENT 'артикул',
    `title` VARCHAR(255) NULL  COMMENT 'название позиции',
    `descr` TEXT NULL  COMMENT 'описание',
...
CREATE TABLE `app_price_cat` (
    ...
    `title` VARCHAR(250) NULL  COMMENT 'название прайса',
...
CREATE TABLE `app_profile` (
    ...
    `clid` INT NULL  COMMENT 'clientcat.clid',
    `value` VARCHAR(255) NULL  COMMENT 'начение поля',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`pfid`),
   INDEX `value` (`value`)
...
CREATE TABLE `app_profile_cat` (
    `id` INT NOT NULL auto_increment,
    `name` VARCHAR(255) NULL  COMMENT 'название поля',
    `tip` VARCHAR(10) NULL  COMMENT 'тип вывода поля',
    `value` TEXT NULL  COMMENT 'значение поля',
    `ord` INT NULL  COMMENT 'порядок вывода',
    `pole` VARCHAR(10) NULL  COMMENT 'название поля для идентификации',
...
CREATE TABLE `app_relations` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  COMMENT 'название',
    `color` VARCHAR(10) NULL  COMMENT 'цвет',
    `isDefault` VARCHAR(6) NULL  COMMENT 'признак по умолчанию',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`),
   INDEX `title` (`title`),
   INDEX `title_2` (`title`)
...
CREATE TABLE `app_reports` (
    ...
    `title` VARCHAR(100) NULL  COMMENT 'название отчета',
    `file` VARCHAR(100) NULL  COMMENT 'файл отчета',
    `ron` VARCHAR(5) NULL  COMMENT 'активность отчета',
    `category` VARCHAR(20) NULL  COMMENT 'раздел',
    `roles` TEXT NULL  COMMENT 'Роли сотрудников с доступом к отчету',
    `users` VARCHAR(255) NULL  COMMENT 'id сотрудников, у которых есть доступ к отчету',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`rid`)
...
CREATE TABLE `app_search` (
    ...
    `tip` VARCHAR(100) NULL  COMMENT 'Привязка к person, client, dog',
    `title` VARCHAR(250) NULL  COMMENT 'Название представления',
    `squery` TEXT NULL  COMMENT 'Поисковой запрос',
    `sorder` INT NULL  COMMENT 'Порядок вывода',
    `iduser` INT NULL  COMMENT 'user.iduser',
    `share` VARCHAR(5) NULL  COMMENT 'Общий доступ',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`seid`)
...
CREATE TABLE `app_settings` (
    `id` INT NOT NULL auto_increment,
    `company` VARCHAR(250) NULL  COMMENT 'название компании',
    `company_full` MEDIUMTEXT NULL ,
    `company_site` VARCHAR(250) NULL  COMMENT 'сайт компании',
    `company_mail` VARCHAR(250) NULL  COMMENT 'email компании',
    `company_phone` VARCHAR(255) NULL  COMMENT 'телефон',
    `company_fax` VARCHAR(255) NULL  COMMENT 'факс',
    `outClientUrl` VARCHAR(255) NULL ,
    `outDealUrl` VARCHAR(255) NULL ,
    `defaultDealName` VARCHAR(255) NULL ,
    `dir_prava` VARCHAR(255) NULL ,
    `recv` MEDIUMTEXT NULL ,
    `gkey` VARCHAR(250) NULL ,
    `num_client` INT NULL  DEFAULT '30',
    `num_con` INT NULL  DEFAULT '30',
    `num_person` INT NULL  DEFAULT '30',
    `num_dogs` INT NULL  DEFAULT '30',
    `format_phone` VARCHAR(250) NULL ,
    `format_fax` VARCHAR(250) NULL ,
    `format_tel` VARCHAR(250) NULL ,
    `format_mob` VARCHAR(250) NULL ,
    `format_dogs` VARCHAR(250) NULL ,
    `session` VARCHAR(3) NOT NULL,
    `export_lock` VARCHAR(255) NULL ,
    `valuta` VARCHAR(10) NULL ,
    `ipaccesse` VARCHAR(5) NULL ,
    `ipstart` VARCHAR(15) NULL ,
    `ipend` VARCHAR(15) NULL ,
    `iplist` MEDIUMTEXT NULL ,
    `maxupload` VARCHAR(3) NULL ,
    `ipmask` VARCHAR(20) NULL ,
    `ext_allow` MEDIUMTEXT NOT NULL,
    `mailme` VARCHAR(5) NULL ,
    `mailout` VARCHAR(10) NULL ,
    `other` MEDIUMTEXT NULL  COMMENT 'все настройки в админ панели',
    `logo` VARCHAR(100) NULL ,
    `acs_view` VARCHAR(3) NULL  DEFAULT 'on',
    `complect_on` VARCHAR(3) NULL  DEFAULT 'no',
    `zayavka_on` VARCHAR(3) NULL  DEFAULT 'no',
    `contract_format` VARCHAR(255) NULL ,
    `contract_num` INT NULL ,
    `inum` INT NULL ,
    `iformat` VARCHAR(255) NULL ,
    `akt_num` VARCHAR(20) NULL  DEFAULT '0',
    `akt_step` INT NULL ,
    `api_key` VARCHAR(255) NULL ,
    `coordinator` INT NULL ,
    `timezone` VARCHAR(255) NULL  DEFAULT 'Asia/Yekaterinburg',
    `ivc` VARCHAR(255) NULL ,
    `dFormat` VARCHAR(255) NULL ,
    `dNum` VARCHAR(255) NULL ,
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_sip` (
    ...
    `sip_port` INT NULL ,
    `sip_channel` VARCHAR(30) NULL ,
    `sip_context` VARCHAR(255) NULL ,
    `sip_user` VARCHAR(100) NULL ,
    `sip_secret` VARCHAR(200) NULL ,
    `sip_numout` VARCHAR(3) NULL ,
    `sip_pfchange` VARCHAR(3) NULL ,
    `sip_path` VARCHAR(255) NULL ,
    `sip_cdr` VARCHAR(255) NULL ,
    `sip_secure` VARCHAR(5) NULL ,
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_smtp` (
    ...
    `smtp_port` INT NULL ,
    `smtp_auth` VARCHAR(5) NULL ,
    `smtp_secure` VARCHAR(5) NULL ,
    `smtp_user` VARCHAR(100) NULL ,
    `smtp_pass` VARCHAR(200) NULL ,
    `smtp_from` VARCHAR(255) NULL ,
    `smtp_protocol` VARCHAR(5) NULL ,
    `tip` VARCHAR(10) NULL ,
...
CREATE TABLE `app_speca` (
    ...
    `prid` INT NULL  DEFAULT '0' COMMENT 'price.n_id',
    `did` INT NULL  DEFAULT '0' COMMENT 'dogovor.did',
    `artikul` VARCHAR(100) NULL ,
    `title` VARCHAR(255) NULL  COMMENT 'название',
    `tip` INT NULL  DEFAULT '0' COMMENT '0 - товар, 1 - услуга, 2 - материал',
    `price` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'цена',
    `price_in` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'закуп',
    `kol` DOUBLE(20,2) NULL  DEFAULT '0.00' COMMENT 'кол-во',
    `edizm` VARCHAR(10) NULL ,
    `datum` TIMESTAMP NULL  DEFAULT CURRENT_TIMESTAMP COMMENT 'дата добавления-корректировки',
    `nds` FLOAT(20,2) NULL  COMMENT 'сумма ндс',
    `dop` INT NULL  DEFAULT '0',
...
CREATE TABLE `app_tasks` (
    `tid` INT NOT NULL auto_increment,
    `maintid` INT NULL  DEFAULT '0' COMMENT 'tasks.tid',
    `iduser` INT NULL  DEFAULT '0' COMMENT 'исполнители user.iduser',
    `clid` INT NULL  DEFAULT '0' COMMENT 'clientcat.clid',
    `pid` VARCHAR(255) NULL  COMMENT 'personcat.pid (может быть несколько с разделением ;)',
    `did` INT NULL  DEFAULT '0' COMMENT 'dogovor.did',
    `cid` INT NULL  DEFAULT '0' COMMENT 'history.cid',
    `datum` DATE NULL  COMMENT 'дата задания',
    `totime` TIME NULL  DEFAULT '09:00:00' COMMENT 'время задания',
    `title` VARCHAR(250) NULL  COMMENT 'название задания',
    `des` TEXT NULL  COMMENT 'описание',
    `tip` VARCHAR(100) NULL  DEFAULT 'Звонок' COMMENT 'тип (откуда пришла работа)',
    `active` VARCHAR(255) NULL  DEFAULT 'yes' COMMENT 'активное или нет напоминание',
    `autor` INT NULL  DEFAULT '0' COMMENT 'автор задания user.iduser',
    `priority` INT NULL  DEFAULT '0' COMMENT 'важность (0-да, 2-нет)',
    `speed` INT NULL  DEFAULT '0' COMMENT 'срочность(0-да, 2-нет)',
    `created` DATETIME NULL  COMMENT 'дата добавление напоминания для исполнителя',
    `readonly` VARCHAR(3) NULL  DEFAULT 'no' COMMENT 'чтение да-нет',
    `alert` VARCHAR(3) NULL  DEFAULT 'yes' COMMENT 'показывать или нет всплывающее окно напоминания',
    `day` VARCHAR(3) NULL  COMMENT 'Признак - напоминание на весь день',
    `status` INT NULL  DEFAULT '0' COMMENT 'Статус выполнения (1-успешно,2-не успешно)',
    `alertTime` INT NULL  DEFAULT '0' COMMENT 'Часы до напоминания, для предварительного уведомления',
...
CREATE TABLE `app_territory_cat` (
    `idcategory` INT NOT NULL auto_increment,
    `title` VARCHAR(250) NULL  COMMENT 'наименование',
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`idcategory`)
...
CREATE TABLE `app_tpl` (
    `tid` INT NOT NULL auto_increment,
    `tip` VARCHAR(20) NULL  COMMENT 'тип',
...
CREATE TABLE `app_uids` (
    `id` INT NOT NULL auto_increment,
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `name` VARCHAR(100) NULL  COMMENT 'название параметра',
    `value` VARCHAR(100) NULL  COMMENT 'знаение параметра',
...
CREATE TABLE `app_user` (
    ...
    `ses` TEXT NULL ,
    `title` VARCHAR(250) NULL  COMMENT 'Имя пользователя',
    `tip` VARCHAR(250) NULL  DEFAULT 'Менеджер продаж',
    `user_post` VARCHAR(255) NULL ,
    ...
    `phone` TEXT NULL  COMMENT 'номер телефона',
    `phone_in` VARCHAR(20) NULL  COMMENT 'добавочный номер',
    `fax` TEXT NULL ,
    `mob` TEXT NULL  COMMENT 'номер сотового',
    `bday` DATE NULL ,
    `acs_analitics` VARCHAR(5) NULL  COMMENT 'Доступ к отчетам',
    `acs_maillist` VARCHAR(5) NULL  COMMENT 'Доступ к рассылкам',
    `acs_files` VARCHAR(5) NULL  COMMENT 'Доступ к разделу Файлы',
    `acs_price` VARCHAR(5) NULL  COMMENT 'Доступ к разделу Прайс',
    `acs_credit` VARCHAR(5) NULL  COMMENT 'Может ставить оплаты',
    `acs_prava` VARCHAR(5) NULL  COMMENT 'DEPRECATED',
    `tzone` VARCHAR(5) NULL  COMMENT 'смещение временной зоны',
    ...
    `acs_import` VARCHAR(255) NULL  COMMENT 'разные права',
    ...
    `avatar` VARCHAR(100) NULL  COMMENT 'аватарка',
    `sole` VARCHAR(250) NULL ,
    `adate` DATE NULL ,
...
INSERT INTO `app_user` VALUES ('1','admin','a9cca450754d8bc1581d573eb09fa66393079060f5b32a3bcaa86d37e617d639ddef9af591c7c022b816729a923650266324370249d1f50012453fa260bf31e975df985456d82921ee3e0ecbf28f96970962321731','s2ys05s9o78o2xONli2ABYabNJZXOhKaXHKVFxIZzqhu3ylcqMeMQW4Rl4ji','Administrator','Руководитель организации','Руководитель','0','0',NULL,'admin',NULL,'0','0',NULL,NULL,NULL,NULL,NULL,'on','on','on','on','on','on','0','on;on;on;on;on;on;on;on;on;on;on;on','d1;d2;d5;d7;d4;d6;d3;d8;d9;d10;d11;d12','yes','on','on;on;on;on;on;on;on;on;on;on;on;on;on;on;on;on;on;on;on','yes','on','0',NULL,NULL,'on;off;off;on;off;on;on;on;on;on;on;on;off;off;off;off;off;off',NULL,'9o78o2xONli2ABYabNJZXaM7CtFFXuvJ',NULL,'{\\\"vigets\\\":{\\\"parameters\\\":\\\"on\\\",\\\"voronka\\\":\\\"on\\\",\\\"analitic\\\":\\\"on\\\",\\\"dogs_renew\\\":\\\"on\\\",\\\"credit\\\":\\\"on\\\",\\\"stat\\\":\\\"on\\\"},\\\"taskAlarm\\\":null,\\\"userTheme\\\":\\\"\\\",\\\"userThemeRound\\\":null,\\\"startTab\\\":\\\"vigets\\\",\\\"menuClient\\\":\\\"my\\\",\\\"menuPerson\\\":\\\"my\\\",\\\"menuDeal\\\":\\\"my\\\",\\\"notify\\\":[\\\"client.add\\\",\\\"client.edit\\\",\\\"client.userchange\\\",\\\"client.delete\\\",\\\"client.double\\\",\\\"person.send\\\",\\\"deal.add\\\",\\\"deal.edit\\\",\\\"deal.userchange\\\",\\\"deal.step\\\",\\\"deal.close\\\",\\\"invoice.doit\\\",\\\"lead.add\\\",\\\"lead.setuser\\\",\\\"lead.do\\\",\\\"comment.new\\\",\\\"comment.close\\\",\\\"task.add\\\",\\\"task.edit\\\",\\\"task.doit\\\",\\\"self\\\"],\\\"filterAllBy\\\":null,\\\"subscribs\\\":null}',NULL,'1');#%%

DROP TABLE IF EXISTS `app_useronline`;#%%
CREATE TABLE `app_useronline` (
    `id` INT NOT NULL auto_increment,
    ...
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
)  COMMENT='Хранилище статусов пользователей: online/offline'  ENGINE=InnoDB DEFAULT CHARSET='utf8';#%%

INSERT INTO `app_useronline` VALUES ('1','2024-05-14 19:42:39','1','online','1');#%%

DROP TABLE IF EXISTS `app_ver`;#%%
CREATE TABLE `app_ver` (
    `id` INT NOT NULL auto_increment,
    `current` VARCHAR(10) NOT NULL COMMENT 'версия',
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата',
   PRIMARY KEY (`id`)
)  COMMENT='Версии приложения'  ENGINE=MyISAM DEFAULT CHARSET='utf8';#%%

INSERT INTO `app_ver` VALUES ('1','2024.1','2024-05-14 19:10:09');#%%

DROP TABLE IF EXISTS `app_webhook`;#%%
CREATE TABLE `app_webhook` (
    `id` INT NOT NULL auto_increment,
    `title` VARCHAR(255) NULL  DEFAULT 'event' COMMENT 'название ',
    `event` VARCHAR(255) NULL  COMMENT 'событие',
    `url` TINYTEXT NULL ,
    `identity` INT NOT NULL DEFAULT '1',
   PRIMARY KEY (`id`)
...
CREATE TABLE `app_ymail_files` (
    ...
    `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'дата',
    `name` VARCHAR(255) NULL  COMMENT 'оригинальное имя файла',
    `file` VARCHAR(255) NULL  COMMENT 'переименнованое имя файла для системы',
...
CREATE TABLE `app_ymail_messages` (
    ...
    `subbolder` VARCHAR(255) NULL ,
    `messageid` VARCHAR(255) NULL ,
    `uid` INT NULL  COMMENT 'внешний id другой системы',
    `hid` INT NULL  COMMENT 'привязска с историей активности (history.cid)',
    `parentmid` VARCHAR(255) NULL ,
    `fromm` MEDIUMTEXT NULL  COMMENT 'имя почтового ящика',
    `fromname` MEDIUMTEXT NULL  COMMENT 'имя ользователя почтового ящика',
    `theme` VARCHAR(255) NULL  COMMENT 'тема сообщения',
    `content` LONGTEXT NULL  COMMENT 'текст сообщения',
    `iduser` INT NULL  COMMENT 'id пользователя user.iduser',
    `fid` TEXT NULL ,
    `did` INT NULL  COMMENT 'id сделки dogovor.did',
    `identity` INT NOT NULL DEFAULT '1',
...
CREATE TABLE `app_ymail_messagesrec` (
    ...
    `name` VARCHAR(200) NULL  COMMENT 'имя отправителя-получателя',
