...
function getTags($identity): array {
	...
	$db       = $GLOBALS['db'];
	$identity = empty( $identity ) ? $GLOBALS['identity'] : $identity;
...
function URL2slack($string): array {
	...
	preg_match_all( "#<a.*href=[\"'](.*)[\"'].*>(.*)</a>#isU", $string, $matchs, PREG_SET_ORDER );
...
class CreateTemplateTag {
	...
	public function Tags($tip, $params): array {
		...
		require_once $prefix."/inc/class/Client.php";
		require_once $prefix."/inc/class/Deal.php";

		$db       = $GLOBALS['db'];
		$identity = $GLOBALS['identity'];
		$sqlname  = $GLOBALS['sqlname'];

		if ( (int)$params['identity'] > 0 ) {
		...
		switch ($tip) {
			...
			case 'person.add':
				...
				$person = get_person_info( (int)$params['pid'], "yes" );
			...
			case 'person.edit':
				...
				$response['link']     = '<a href="'.$scheme.$server.'/card.person.php?pid='.$params['pid'].'">Ссылка</a>';
				$response['url']      = $scheme.$server.'/card.person.php?pid='.$params['pid'];
			...
			case 'deal.edit':
				...
				$deal = Salesman\Deal ::info( (int)$params['did'] );
				...
				$response['pid']        = (int)$deal['person']['pid'];
				$response['person']     = $deal['person']['person'];
				$response['idstep']     = (int)$deal['step']['stepid'];
				$response['step']       = $deal['step']['steptitle'];
				$response['iduser']     = (int)$deal['iduser'];
				$response['user']       = $deal['user'];
				$response['idautor']    = (int)$params['autor'];
			...
			case 'deal.change.step':
				...
				$response['pid']        = (int)$deal['person']['pid'];
				$response['person']     = $deal['person']['person'];
				$response['step']       = (int)$params['stepNew'];
				$response['stepOld']    = (int)$params['stepOld'];
				$response['idautor']    = (int)$params['autor'];
				$response['autor']      = current_user( $params['autor'], "yes" );
			...
			case 'invoice.expressadd':
				...
				$response['deal']['client']     = $deal['client']['title'];
				$response['deal']['idstep']     = (int)$deal['step']['stepid'];
				$response['deal']['step']       = $deal['step']['steptitle'];
				$response['deal']['datum_plan'] = format_date_rus( $deal['datum_plan'] );
				$response['deal']['iduser']     = (int)$deal['iduser'];
				$response['deal']['user']       = $deal['user'];
			...
			case 'lead.add':
				...
				$response['idclientpath'] = (int)$lead['clientpath'];
				$response['clientpath']   = ((int)$lead['clientpath'] > 0) ? current_clientpathbyid( (int)$lead['clientpath'] ) : '--не определен--';
			...
			case 'lead.setuser':
				...
				$response['link']   = '<a href="'.$scheme.$server.'/leads?id='.$params['id'].'">Ссылка</a>';
				$response['url']    = $scheme.$server.'/leads?id='.$params['id'];
				$response['client'] = ((int)$lead['clid'] > 0) ? current_client( (int)$lead['clid'] ) : "";
				$response['person'] = ((int)$lead['pid'] > 0) ? current_person( (int)$lead['pid'] ) : "";
			...
			case 'lead.do':
				...
				$response['client'] = ((int)$lead['clid'] > 0) ? current_client( (int)$lead['clid'] ) : "";
				$response['person'] = ((int)$lead['pid'] > 0) ? current_person( (int)$lead['pid'] ) : "";
				$response['deal']   = ((int)$lead['did'] > 0) ? current_dogovor( (int)$lead['did'] ) : "";
				$rez                = (int)$lead['rezult'];
			...
			case 'entry.add':
				...
				$response['autor']  = current_user( (int)$params['autor'] );
				$response['user']   = current_user( (int)$response['iduser'] );
				$response['link']   = '<a href="'.$scheme.$server.'/entry?id='.$params['id'].'">Ссылка</a>';
				$response['url']    = $scheme.$server.'/entry?id='.$params['id'];
				$response['client'] = ((int)$response['clid'] > 0) ? current_client( (int)$response['clid'] ) : "";
				$response['person'] = ((int)$response['pid'] > 0) ? current_person( (int)$response['pid'] ) : "";
				$response['deal']   = ((int)$response['did'] > 0) ? current_dogovor( (int)$response['did'] ) : "";
