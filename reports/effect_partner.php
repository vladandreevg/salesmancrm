...
$res = $db -> getAll( "SELECT * FROM ".$sqlname."clientcat WHERE type = 'partner' and identity = '$identity' ORDER by title" );
foreach ( $res as $data ) {

	$string = '';
	$result = $db -> getAll( "SELECT * FROM ".$sqlname."dogprovider WHERE partid = '".$data[ 'clid' ]."' and identity = '$identity'" );
	foreach ( $result as $datad ) {

		//if($datad['did'] < 1) goto ex;

		//получим данные по сделке
		$dataar   = get_dog_info( $datad[ 'did' ], 'yes' );
		...
		$toplist[ $datad[ 'partid' ] ] += $datad[ 'summa' ];
...
<div class="relativ mt20 mb20 wp95" align="center">
	<h1 class="uppercase fs-14 m0 mb10">Партнеры по сделкам</h1>
