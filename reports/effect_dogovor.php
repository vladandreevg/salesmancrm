...
foreach ( $fields as $i => $field ) {
	...
	if ( !in_array( $field, $ar ) && !in_array( $field, [
		...
		$sort2 .= " deal.{$field} = '".$field_query[ $i ]."' AND ";
	}
	elseif ( $field == 'close' ) {
		$sort2 .= $field_query[ $i ] != 'yes' ? " COALESCE(deal.{$field}, 'no') != 'yes' AND " : " COALESCE(deal.{$field}, 'no') == 'yes' AND ";
	}
	elseif ( $field == 'mcid' ) {
		$mc = $field_query[ $i ];
	}
...
foreach ( $result as $data_array ) {
	...
	$kol_c = ( $plan_form == 'datum_close' ) ? $result6[ 'kol_fact' ] : $result6[ 'kol' ];
	...
	$datas[] = '{"Тип":"'.$tip_dogovor.'","Статус":"Активно","Кол-во":"'.$new_d.'"}';
	$datas[] = '{"Тип":"'.$tip_dogovor.'","Статус":"Закрыто","Кол-во":"'.$cl_d.'"}';
...
<div class="zagolovok_rep text-center">
	<b>Анализ сделок<b class="red">по типам</b> за период&nbsp;с&nbsp;<?= format_date_rus( $da1 ) ?>&nbsp;по&nbsp;<?= format_date_rus( $da2 ) ?></b>
</div>
...
<?php if ( $count > 0 ) { ?>
	...
	<div id="graf" style="display:block; height:350px">
		...
		<script>
			...
			var x = myChart.addCategoryAxis("x", ["Тип", "Статус"]);
			...
			myChart.setMargins(50, 20, 60, 135);
...
<table class="wp95">
	...
	<TR height="40">
		...
		<th align="center"><B>Тип сделки</B></th>
	...
	<TBODY>
	<?php
	foreach ($effect as $j => $row) {
		?>
		<TR height="40" class="ha">
			<TD align="center"><?= $j + 1 ?></TD>
			<TD>
				<DIV class="ellipsis" title="<?= $row[ 'dogovor' ] ?>"><?= $row[ 'dogovor' ] ?></DIV>
			</TD>
			<TD align="center">
				<DIV title="<?= $row[ 'new_dogs' ] ?>"><?= $row[ 'new_dogs' ] ?></DIV>
			</TD>
			<TD align="right" nowrap>
				<DIV title="<?= num_format( $row[ 'kol_new' ] ) ?>"><?= num_format( $row[ 'kol_new' ] ) ?></DIV>
			</TD>
			<TD align="center">
				<DIV title="<?= $row[ 'close_dogs' ] ?>"><?= $row[ 'close_dogs' ] ?></DIV>
			</TD>
			<TD align="right" nowrap>
				<DIV title="<?= num_format( $row[ 'kol_close' ] ) ?>"><?= num_format( $row[ 'kol_close' ] ) ?></DIV>
			</TD>
			<TD align="right" nowrap>
				<DIV title="<?= num_format( $row[ 'marga' ] ) ?>"><?= num_format( $row[ 'marga' ] ) ?></DIV>
			</TD>
		</TR>
	<?php } ?>
