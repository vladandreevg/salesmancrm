...
foreach ( $resultt as $data ) {
	...
	foreach ( $user_list as $iduser ) {
		...
		$resultm = $db -> getRow( "
			SELECT 
				COUNT(deal.did) as count, SUM(deal.marga) as summa
...
<DIV class="zagolovok_rep text-center">
	<b>Воронка продаж (по марже)&nbsp;с&nbsp;<span class="red"><?= format_date_rus( $da1 ) ?></span>&nbsp;по&nbsp;<span class="red"><?= format_date_rus( $da2 ) ?></span></b>:
</DIV>
<hr>
