...
$kolSum_fact = $kolSum = 0;

$da = $db -> getAll("SELECT * FROM ".$sqlname."dogovor `deal` WHERE deal.did > 0 and $sort deal.datum between '$da1 00:00:00' and '$da2 23:59:59' and deal.identity = '$identity' ORDER BY deal.datum_plan");
foreach ($da as $data) {
...
if ($_REQUEST['action'] == "get_csv") {
	...
	$otchet = array("#;Дата создан.;Дата план.;Заказчик;Сделка;Источник клиента;Ответств.;Описание;Сумма сделки, р.;Этап сделки;Результат/Причина;Сумма результ., р.;Дата закрыт.");
	...
	foreach ($dogs as $dog) {
		...
		$otchet[] = $j.'.;"'.$dog['datum'].'";"'.$dog['datum_plan'].'";'.$client.';'.current_dogovor($dog['did']).';'.$dog['clientpath'].';'.$dog['manager'].';'.preg_replace("/\r\n|\r|\n/u", "", untag($dog['des'])).';"'.pre_format($dog['kol']).'";"'.$dog['step'].'%";'.preg_replace("/\r\n|\r|\n/u", "", current_dogstatus($dog['did'])).';'.num_format($dog['kol_fact']).';"'.$dog['datum_fact'].'"';
...
<div class="relativ mt20 mb20 wp95" align="center">
	<h1 class="uppercase fs-14 m0 mb10">Информация по сделкам</h1>
...
<TABLE width="99%" align="center" cellpadding="5" cellspacing="0">
	...
	<TR class="header_contaner" height="30">
		...
		<td width="80" align="center"><b>Источник клиента</b></td>
		<td width="80" align="center"><b>Ответств.</b></td>
		<td width="200" align="center"><b>Активности</b></td>
		<td width="90" align="center"><b>&sum; сделки, <?= $valuta ?></b></td>
		<td width="70" align="center"><b>Этап сделки</b></td>
		<td width="100" align="center"><b>Результат / Причина</b></td>
		<td width="90" align="center"><b>&sum; результ., <?= $valuta ?></b></td>
		<td width="70" align="center"><b>Дата закрыт.</b></td>
	</TR>
	</thead>
	<?php
	foreach ($dogs as $i => $dog) {
		...
		<TR class="ha <?= $dog['color'] ?>" height="40px">
			...
			<TD align="center"><?= format_date_rus($dog['datum']) ?></TD>
			...
			<TD>
				<div class="dot-ellipsis" title="<?= html2text($dog['des']) ?>"><?= nl2br($dog['des']) ?></div>
			</TD>
			<TD align="right"><?= num_format($dog['kol']) ?></TD>
			<TD align="right"><?= $dog['step'] ?>%</TD>
			<TD align="left"><?= current_dogstatus($dog['did']) ?></TD>
			<TD align="right"><?= num_format($dog['kol_fact']) ?></TD>
			<TD align="center"><span class=""><?= $dog['datum_fact'] ?></span></TD>
		</TR>
	<?php } ?>
	<TR bgcolor="#FFCC33" height="30">
		...
		<td align="center"></td>
		<td align="center"></td>
		<td align="right"><b class="smalltxt"><?= num_format($kolSum) ?></b></td>
		<td align="center"></td>
		<td align="center"></td>
		<td align="right"><b class="smalltxt"><?= num_format($kolSum_fact) ?></b></td>
		<td align="center"></td>
	</TR>
</TABLE>
