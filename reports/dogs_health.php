...
foreach ( $da as $data ) {
	...
	if ( $complect_on == 'yes' ) {
		...
		if ( $dmin != '' && diffDate2( $datum_min, $dmin ) > 0 )
			$datum_min = $dmin;
	...
	while ($datac = $db -> fetch( $resultc )) {
		...
		if ( diffDate2( $datum_min, $datac['datum_credit'] ) > 0 && $datac['do'] != 'on' && $datac['datum_credit'] != '0000-00-00' )
			$datum_min = $datac['datum_credit'];
		if ( diffDate2( $datum_min, $datac['invoice_date'] ) > 0 && $datac['do'] != 'on' && $datac['invoice_date'] != '0000-00-00' )
...
<TABLE id="zebra">
	...
	<TR class="header_contaner">
		<td class="w70"><b>Дата план.</b></td>
		<td class="w40"></td>
		<td class="w60"></td>
		<td class="w250"><b>Название сделки</b></td>
		<td class="w100"><b>Стоимость, <?= $valuta ?></b></td>
		<td class="w100"><b>Оплата, <?= $valuta ?></b></td>
		<td><b>Заказчик</b></td>
	</TR>
	</thead>
	<?php
	foreach ( $dogs as $dog ) {
		?>
		<TR class="ha">
			<TD class="text-center"><?= format_date_rus( $dog['datum_plan'] ) ?></TD>
			...
			<TD class="text-right"><?= num_format( $dog['summa'] ) ?></TD>
			<TD>
				<div class="ellipsis">
					<a href="javascript:void(0)" onclick="openClient('<?= $dog['clid'] ?>')" title="Открыть">
