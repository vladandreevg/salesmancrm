...
$user_list = (!empty( $user_list ) && (int)$user_list[0] > 0) ? $user_list : (array)get_people( $iduser1, "yes" );
...
foreach ( $fields as $i => $field ) {
	...
	if ( !in_array( $field, $ar ) && !in_array( $field, [
		...
		$sort .= " dg.".$field." = '".$field_query[ $i ]."' AND ";
	}
	elseif ( $field == 'close' ) {
		$sort .= $field_query[ $i ] != 'yes' ? " COALESCE(dg.{$field}, 'no') != 'yes' AND " : " COALESCE(dg.{$field}, 'no') == 'yes' AND ";
	}
	elseif ( $field == 'mcid' ) {
		$mc = $field_query[ $i ];
	}
...
if ( $action == "export" ) {
	...
	FROM ".$sqlname."credit `cr`
		LEFT JOIN ".$sqlname."dogovor `dg` ON cr.did = dg.did
		LEFT JOIN ".$sqlname."clientcat `cl` ON cl.clid = cr.clid
		LEFT JOIN ".$sqlname."user `us` ON us.iduser = cr.iduser
		LEFT JOIN ".$sqlname."direction `dir` ON dg.direction = dir.id
		LEFT JOIN ".$sqlname."dogtips `tp` ON dg.tip = tp.tid
	WHERE
		cr.crid > 0 and
		$sort
	...
	foreach ( $da as $data ) {
		...
		$list[] = [
			...
			"idatum"    => $data['idatum'] != "0000-00-00" ? $data['idatum']." 12:00:00" : "",
			"datum"     => $data['datum'] != "0000-00-00" ? $data['datum'] : "",
			"summa"     => $data['summa'],
			"dsumma"    => $data['dsumma'],
			"dmarga"    => $data['dmarga'],
			"deal"      => $data['dogovor'],
			...
			"client"    => $data['client'],
			"user"      => $data['user'],
			"dfact"     => ($data['do'] == 'on') ? $data['dfact']." 12:00:00" : '',
	...
	*/
	$templateFile = 'templates/invoicesTemp.xlsx';
	$outputFile   = 'exportInvoices.xlsx';
...
if ( !$action ) {
	...
	$statusesClose = Guides ::closeStatusPlus();
	...
	foreach ( $da as $data ) {
		...
		if ( $data['close'] == 'yes' && $data['do'] != "on" ) {

			//$status = $statusesClose[$data['sid']];

			$data['status'] = '<span class="gray">Отменен</span>';
	...
	<div class="relativ mt20 mb20 wp95 text-center">
		<h1 class="uppercase fs-14 m0 mb10">Выставленные счета по сотрудникам</h1>
	...
	<div class="block mt10">
		...
		<TABLE>
			...
			<TR class="th35">
				...
				<th class="w120 text-center"><b>Дата план</b></th>
				<th class="w120 text-center"><b>Дата факт</b></th>
				<th class="w120 text-center"><b>Сумма</b></th>
				<th class="text-center"><b>Сделка</b></th>
				<th class="text-center"><b>Компания</b></th>
			...
			foreach ( $list as $user => $udata ) {
				...
				print '
				<tr class="td--main bluebg-sub sticked--top">
					<td colspan="9"><div class="Bold blue fs-11 inline">'.$users[ $user ].'</div> [ Сумма: <span class="Bold gray2">'.num_format( $summas[ $user ] ).'</span> ]</td>
				</tr>
				';
				...
				foreach ( $udata as $datum => $val ) {
					...
					$diff = (get_date( $val['dfact'] ) == '') ? '<sup class="gray2">'.diffDate2( $val['dplan'] ).' дн.</sup>' : '';
					...
					<tr class="datetoggle td--main '.$color.'" data-key="'.$user.'">
						...
						<td><b>'.$val['status'].$diff.'</b></td>
						<td class="text-right">'.get_date( $val['dplan'] ).'</td>
						<td class="text-right">'.get_date( $val['dfact'] ).'</td>
						...
						<td><div class="ellipsis" title="'.($val['mc']).'">'.($val['mc']).'</div></td>
